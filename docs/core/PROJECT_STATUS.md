...
**Last Updated:** September 11, 2026 (v3.2.58)
**Current Phase:** Beta — live in production
**Current Version:** **3.2.58 — PENDING DEPLOY.** Live is v3.2.57: `/health` read `f926d5b` at 2026-09-11 10:54Z. **Trust `/health` over this line**: status lines have gone stale within hours more than once. 3.2.46 stays permanently skipped.
...
## Health
- **Tests (v3.2.58):** `npx vitest run`, the whole suite including ghost: **3170/3170 across 217 files**, green on the first attempt. Typecheck ✅, production build ✅. (v3.2.56's first full run failed 2 fixtures that expected the old "Bank:" label; both were updated.)
...
- **Deploy:** ⏳ v3.2.58 pending; v3.2.57 live (see above). `bash deploy.sh` is Tom's to run, from a Windows terminal. A stock CSV change also re-bakes every classroom's `resolved/` copy on the next boot, so never hand-edit those.
