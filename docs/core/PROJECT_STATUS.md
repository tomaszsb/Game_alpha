...
1. **Browser smoke of v3.0.78** (server verified live; UI not yet eyeballed). Vite 8 = new bundler → confirm the game loads + plays a turn. Then the teacher flow: Admin Tools → 👥 Manage Classrooms & Teachers → create teacher + classroom → log in as that teacher → start a game on their board.
2. **Phase 4 — card insertion** (teacher-authored spaces). **Now unblocked** (Phase 3 live). Most invasive; design it next.
