...
## Health
- **Tests:** full vitest run **confirmed green — 2173 pass / 1 skip / 1 fail** (the 1 fail is the known Windows `rmdir`-race teardown flaky in `instanceStore.test.ts`, body passes, green in isolation — not a regression). Targeted sweep 1661/1661, server 250/250, new ghost fixture 3/3 all green. Typecheck + build clean.
