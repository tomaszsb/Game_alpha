...
**Current Version:** **3.1.80** — pushed to `master`, **not yet deployed**. Last confirmed-live commit is `81c259f` (v3.1.76-era), bundle-grepped 2026-07-30. Four versions stacked since (clipboard-copy fix, CSP/HSTS/Permissions-Policy headers, dead-code cleanup, this session's lint-rule refactor) — deploy whenever ready.
...
- **Deploy:** live = commit `81c259f` (v3.1.76-era), verified 2026-07-30. **v3.1.77–3.1.80 pushed but not deployed** — run `ssh unraid "cd /mnt/user/appdata/Game_alpha && bash deploy.sh"` when ready. (`/health`'s version field remains an unreliable "dev" placeholder — grep the bundle for the git commit.)
- **Dashboard feedback:** **8 open** as of last check (2026-07-30), all tracked in TODO.md. `fb:2948cf19` (clipboard-copy) shipped this session (v3.1.77) — flip queued in `.claude/fixloop/flip-queue.txt`, pending deploy confirmation.
...
1. **Deploy v3.1.77–3.1.80** — four versions pushed and verified, sitting undeployed.
