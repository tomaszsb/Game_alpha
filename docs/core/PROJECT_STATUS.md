...
- **Deploy:** fully live = commit `a6c2b47` / `3.1.75`, verified 2026-07-29 by grepping the served bundle for both. (`/health`'s version field remains an unreliable "dev" placeholder — grep the bundle, and when a version bump lands *after* a deploy, grep for the git commit instead.) The **`deploy.sh` race fix is live and worked on its first real run**: the container carries `--read-only` and `--cap-drop ALL` — flags only `deploy.sh` passes — so the script's own `docker run` created it, unlike the previous deploy where Unraid's template recreated it mid-build. Its image ID matches the built image, so the new verification step passed too.
