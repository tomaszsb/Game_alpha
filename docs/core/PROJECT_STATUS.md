...
**Current Version:** **3.0.72** (committed + pushed, **pending deploy**; v3.0.71 is live)
...
- **Deploy:** v3.0.72 NOT yet deployed (user runs it). No server config needed — `ADMIN_PASSWORD_HASH` + `FEEDBACK_TOKEN` already set in production.

## Top open items (full list in TODO.md + .claude/NEXT_SESSION.md)
1. **Deploy v3.0.72** — the locks aren't live until then. Dashboard side is already prepared.
2. **Onboarding package** (`fb:0aa9660c` + `fb:8ad42b52` + `fb:f22035af`) — game-level tutorial; the biggest remaining product lever (recurring "overwhelming for newcomers" feedback theme).
3. **Teacher instance layer + space catalog** (design initiative) — master-library vs per-instance-config split; needs a focused design session first. Next audit items after that: DEF-3 (hook-order crash risk) and DEF-4 (lint rehab).
