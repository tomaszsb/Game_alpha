...
**Last Updated:** May 23, 2026 PM
**Current Phase:** Beta — live in production
**Current Version:** **3.0.8** (pending deploy at session end). Twelve versions shipped May 22–23 morning to v3.0.0; eight more shipped May 23 afternoon driven by fresh dashboard feedback against the v3.0.0 build: v3.0.1 scoped the `window.error` handler so the ResizeObserver loop warning stops tripping the red fallback (`fb:241911b6`); v3.0.2 restored the version/commit sync-status pill on the setup screen and lifted the v2.70.1 dice-collapse rule into a tested helper; v3.0.3 swept the voice rule across four sister sections (TimeSection, ProjectScopeSection, FinancesSection, EventsSection) that v2.61.1 had missed (`fb:b1a52932/36ad2471`); v3.0.4 made `StateService.calculateRequiredActions` count unresolved choice-movement so the End Turn tooltip stops lying (`fb:ffff07e2`); v3.0.5 ran the same voice sweep through notifications, the player log, the Negotiation modal, and the Educational empty state (`fb:7a99da1a/004dc390`); v3.0.6 added NPC speaker attribution to `DiceService.generateEffectSummary` — `"The Owner: You took on 2 work packages."` instead of `"Good news! You took on…"` — with PM-voiced first-person at the five PM-narrated spaces (`fb:c3e5322b/94c374d8/44a4eb47`); v3.0.7 embedded `{fundingAmount}` token interpolation in space stories so the Owner quotes the dollar figure inside their dialogue (`fb:61a85444`); v3.0.8 added a one-shot nudge-and-glow animation on the ledger side pill when it enters a gap/surplus state (`fb:fae27391`).
