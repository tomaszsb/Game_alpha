...
**Current Version:** **3.1.10** — committed + pushed, **pending deploy** (live is v3.1.8). v3.1.9: the orphaned classic-panel dead-code cluster deleted (−2,839 lines — CardModal/CardActions/CardContent/CardsSection/PlayerActionService/showCardModal + the `activeModal` state field). v3.1.10: latent N×N card-effect fan-out on the manual `playCard` path fixed (Kid E guard extended to all paths, red-green regression test).
...
- **Deploy:** live = v3.1.8 (confirmed 2026-07-18). v3.1.9 + v3.1.10 committed + pushed, awaiting deploy.

## Top open items (full list in TODO.md + .claude/NEXT_SESSION.md)
1. **Deploy v3.1.9+v3.1.10** — pushed, ready; hand the deploy command to the maintainer.
2. **Real-TV confirmation of the v3.1.2 camera fix** — still outstanding; needs a glance on the physical TV.
3. **Dark/light mode coverage beyond `PlayerPanelV2`** — board, `TVDisplay`, `ChoiceModal` still light-only (`CardModal`, the other dark-unaware modal, was deleted in v3.1.9 — scope shrank).
