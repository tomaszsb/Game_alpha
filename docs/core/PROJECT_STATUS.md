...
**Current Version:** **3.1.8** — **deployed, confirmed live** (bundle hash `index-CyJDO-6t.js` matches the local v3.1.8 build exactly). Bundles v3.1.5–v3.1.8: the domain-event architecture (all 4 stages) + a real bankruptcy/design-fee-cap toast bug fix + the WIN ending now reaching log/toast/TV for the first time.
...
- **Deploy:** live = v3.1.8, confirmed 2026-07-18 via bundle hash match. v3.1.5–v3.1.8 all shipped in this deploy.

## Top open items (full list in TODO.md + .claude/NEXT_SESSION.md)
1. **Delete an orphaned dead-code cluster** — `CardModal.tsx`/`CardActions.tsx`/`CardsSection.tsx`+`.css`/`PlayerActionService.playCard()`/`StateService.showCardModal()`. Audited 2026-07-17: `showCardModal` (the only trigger for `CardModal`) has zero callers anywhere in `src/` — a leftover from the v3.0.128-137 classic-panel removal. Contains a real but unreachable bug; safe, cheap cleanup, not yet done.
