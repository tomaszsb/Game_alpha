...
**Current Version:** **3.0.98** — **deployed and verified live** (deployed bundle confirms `version:"3.0.98"`, `gitCommit:"7ed1dbf"`, matching HEAD exactly — no drift).
...
## Health
- **Tests:** typecheck ✅ clean, build ✅ clean. Full suite initially looked hung on the known Windows ghost-test gate but actually just took ~25 minutes (`tests/ghost/`) — it finished clean: **2339/2340 passing, 1 skipped**.
- **Lint:** ~386 pre-existing errors (DEF-4, long-standing).
- **Deploy:** **v3.0.98 deployed** 2026-07-07 (`bash deploy.sh` from a Windows terminal). Verified live via the deployed bundle's embedded version/commit strings.
...
2. **Board-editor drag-overlap fix needs a real mouse-drag confirmation** — verified via 6 unit tests + code review only; synthetic pointer events don't register with React Flow's drag system in this session's browser automation, so nobody has watched it happen live yet.
3. **Remaining design backlog from the "New-panel playtest" TODO section** — expeditor-timing gate, outcome-modal scope changes, and the plan-examiner-verdict + DOB/FDNY approval-moment feature. See TODO.md.
