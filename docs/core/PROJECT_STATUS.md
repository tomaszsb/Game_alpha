...
**Last Updated:** September 12, 2026 (v3.2.59)
**Current Phase:** Beta — live in production
**Current Version:** **3.2.59 — PENDING DEPLOY.** v3.2.58 IS live: `/health` read `8c8cfea` at 2026-09-12 11:21Z (Tom deployed it on 09-11; the previous snapshot's "pending" line was stale within hours — trust `/health`, not this file). **Trust `/health` over this line**: status lines have gone stale within hours more than once. 3.2.46 stays permanently skipped.
...
## Health
- **Tests (v3.2.59):** `npx vitest run`, the whole suite including ghost: **3171/3171 across 218 files**, green on the first attempt. Typecheck ✅, production build ✅. (v3.2.56's first full run failed 2 fixtures that expected the old "Bank:" label; both were updated.)
...
- **Deploy:** ⏳ v3.2.59 pending; v3.2.58 live (see above). `bash deploy.sh` is Tom's to run, from a Windows terminal. A stock CSV change also re-bakes every classroom's `resolved/` copy on the next boot, so never hand-edit those.
