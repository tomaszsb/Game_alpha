...
**Last Updated:** June 8, 2026 (Session **v3.0.70** — Phase 2.2 TurnTransaction boundary [state TEMP/REAL + log session bundled into begin/commit/discard on TurnService, anti-drift test]; smart-bot calibration → 90% goal MET [deterministic 47/50 once games finish; floor ≥43; `perGameTimeoutMs` opt-in; `.claude/ghost-history.jsonl` logging]; cheat-space fb:89d9f101(b) verified already-fixed + locked `shouldShowMovementDiceButton`; 3 safe `any` narrowings.)
**Charter Version:** 3.34 (added: ghost win-rate is timeout-contaminated until games finish — 30s cap aborts grindy games as losses, raise `perGameTimeoutMs` per-test for a deterministic number; vitest swallows `console.log` on a PASSING test → file-log results via `appendFileSync` before the asserts; audit-before-refactor caught a stale TODO again — verify the bug still repros, then lock the untested seam. Prior 3.33: ghost win-rate drop is a bot-policy question before a balance one — blind Try Again at examiner spaces reverts just-earned approval [TEMP] → Stage-1 gate loop; split coverage [0-hard-failures] from win-rate [smart bot]; hoisting a call out of a short-circuited `&&` runs it for other branches.)
