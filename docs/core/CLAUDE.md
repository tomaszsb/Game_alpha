...
**Last Updated:** June 14, 2026 (Session 2026-06-14 — **v3.0.78 built, pending deploy**. Fixed the two deploy/migration bugs (v3.0.77, deployed+verified) + **proved the data-deploy gap dead** (live sentinel reached origin). Built teacher layer **Phase 3** end-to-end (accounts/sessions, multi-classroom, combined-login UI — no third-party auth). Upgraded **Vite 7→8** → 0 audit vulns. Phase 3d = deploy+verify, then Phase 4 unblocks.)
**Charter Version:** 3.40 (added: teacher-layer Phase 3 multi-teacher front door — scrypt accounts/sessions, ownership-in-guard, combined login, per-instance `/data/i/<id>/` serving + the Express route-ordering trap; major-build-tool-upgrade live-smoke rule (Vite 8/Rolldown — tests pass vs source, bundle can still break). Prior 3.39: deploy not teacher-layer-aware. The "CSV data fixes do NOT reach live" recipe above is now OBSOLETE — gap proven dead v3.0.77.)
