...
- **Testing:** `npm test` must be green — that is the real gate (198 files). `./tests/scripts/run-tests-batch-fixed.sh` runs that same set in chunks with readable per-directory progress: since 2026-08-30 it **derives** its file list from the same include/exclude rules as `vitest.config.dev.ts` instead of enumerating files by hand, and every run ends by asserting the set it dispatched IS the set it discovered. (It used to enumerate 60 of 198, so 138 files across 19 directories were invisible to it and a regression in `tests/components/classroom/` passed "22/22 batches" — that is why the check is set equality, not a count: the first cut of the rewrite dropped one file per directory and duplicated another, keeping the total at exactly 198 while running 179 distinct files.) `DRY_RUN=1` lists without running; `BATCH_SIZE` / `BATCH_TIMEOUT` tune it. Use the batch runner mid-session; run `npm test` before committing. Ghost Player strict + try-again-happy variants both pass (`npm run test:ghost`, ~15 min, run separately since v3.1.11).
...
### Five reusable findings from the Bank Review session (2026-09-25, v3.2.76)

**1. A number authored in the words but missing from the game is often thrown away by the data step, not by the engine.** Bank Review's Time column said "1 day per $200K" and every visit charged 1: `processGameData.js` stripped "day(s)" and `parseInt`ed the rest. The engine already had a half-built vocabulary for it (`add_per_amount` + `per_200k` in `SpaceEffectService`, `per_*` let through by `ConditionEvaluator`) that the live path could not reach — it only serves MANUAL time rows, while this row is `auto` and charged on leaving through `EffectFactory`, which ignores conditions. Fix shape: keep `effect_action=add`, put the "per how much" in the row's `condition` as `per_<amount>` (an unaware consumer then charges the old fixed number instead of dropping to 0), read it back with `perAmountUnit`/`timeRowDays` in `costPreview.ts`, and settle the row into a plain day count in `TurnEffectsOrchestrator` before `EffectFactory`. Before adding a new engine mechanism for an authored phrase, grep for the phrase's own token (`per_200k`) — the vocabulary may already exist.

**2. "The loan on the table" = current loans minus the turn-start snapshot's loans (`getLoanOnTheTable`).** Loans STACK at Bank Review; the fee is charged on ARRIVAL (on the loans you already had) and the time on LEAVING (after the draw), so the two use different moments of one visit. The push-back price must be read BEFORE `discardTurnTransaction` — the TEMP rollback discards the very loan being priced.

**3. To measure what real games reach, replay the smart-bot batch with a spy — on a frozen copy of the data.** Copy `public/data/CLEAN_FILES/*.csv` to a scratch dir, pass it to `bootstrapHeadlessServices(dir)`, seed `mulberry32(100001 + i)` exactly like `runGhostBatch` (options: `maxTurns 300, tryAgainProbability 0.2, smartTryAgain, detectLoops`, 120 s cap), and wrap `EffectEngineService.prototype.processEffects` (source `space_arrival`/`space_leaving`) and `TurnService.prototype.tryAgainOnSpace`. Reproducing the recorded 49/50 and 70.1 turns is the proof the copy is faithful; run it before AND after the change (~30 min each in the background; a full test suite alongside did not disturb it). A temporary test under `tests/ghost/` would be swept into `npm run test:ghost` — delete it before the gates run.

**4. An exact-days test must call the leaving step, not a whole End Turn.** `endTurnWithMovement` also ARRIVES at the next space, where a random life event can add days; four of five test cases passed by luck. Call `(turnService as any).processLeavingSpaceEffects(playerId, space, visit)` directly for the exact charge and keep one full End Turn assertion as `>=`.

**5. Browser-verifying an arrival effect needs a real arrival.** Teleporting a dev player (`updatePlayer({currentSpace})`) skips `startTurn`/auto-funding, so Owner's Money showed a blank amount on the panel too; played through the normal first turn the same space read "$250,000". Also: a relayed brief's data paths may point at `server/data/game-data/` — a git-ignored, stale runtime copy (it lacked a column the repo had); the real source is `public/data/`.

### Four reusable findings from the push-back session (2026-09-24, v3.2.74–75)

**1. A `time/add` row in `SPACE_EFFECTS.csv` is charged on End Turn too — it is not a "push-back price."** Investor Review / Hire a Builder / Final Approval have NO fixed time row (their time is a dice roll), so a push-back there charged 0 and threw away a bad roll for a free re-roll. Typing days into the effects table would have taxed every player who does *not* push back on top of the roll. The price is its own column, `try_again_days` (`Spaces.csv` → `SPACE_CONTENT.csv`), and `calculatePushBackDays(effects, content)` in `costPreview.ts` is the single rule the engine, the cost box and the engagement count all read. Blank = fixed rows; a number replaces them; a malformed cell reads as *no price*, never 0.

**2. Adding a per-space data column — the safe recipe.** (a) Run `node scripts/regen-clean-files.mjs` on the UNMODIFIED tree first: a clean `git diff` proves the pipeline reproduces CLEAN, so regenerating later can only change what you meant. (b) Append the column to `SOURCE_FILES/Spaces.csv` with a record-aware script (quoted multi-line cells) and prove every original cell is unchanged. (c) Add it to `processSpaceContent` (`processGameData.js`), regenerate, and confirm ONLY `SPACE_CONTENT.csv` changed. The Space Editor (`_extraColumns`) and the classroom bake (`applyConfigToSpacesCsv`) already keep unknown columns and a pre-column teacher copy falls through to the stock value — both pinned by test, so no editor/resolver code is needed. `DataService` reads by column name, so a trailing column is safe for older files. Data reaches live on a normal deploy; verify by fetching the live `SPACE_CONTENT.csv`.

**3. Reword a label the nightly robot reads → check the Jarvis script.** It keys on `data-actionable` for what it may hold, but `TAB_SUFFIX_RE` in `game_playtest.py` still strips the literal aria-label ending "— tap to compare…" to get a clean caption. A copy pass on `TurnCommitControl`'s label leaves that ending on the captions until the regex is widened. That repo is not this one — hand it to the Jarvis session.

**4. In Git Bash, `sed -i` flips a CRLF file to LF just like the Edit tool does** (`docs/user/USER_MANUAL.md` became a 782-line diff from a one-line change). Use a Python byte-level replace and check `git diff --stat` after; and never trust a Playwright mouse press until the element is scrolled into view (`scrollIntoViewIfNeeded`) — an off-screen press silently does nothing.

### A `uiStrings.ts` default change can silently do nothing — `UI_STRINGS.csv` overrides it (2026-09-22, v3.2.73)

Renaming "Rules" → "How to play", changing `DEFAULT_UI_STRINGS['RULES.title']` in `uiStrings.ts` had zero visible effect — the modal still said "Rules" after a full reload. `getUIString()` checks `UI_STRING_OVERRIDES.get(key) ?? DEFAULT_UI_STRINGS[key] ?? key`, and `public/data/CLEAN_FILES/UI_STRINGS.csv` (loaded at boot into `UI_STRING_OVERRIDES`) already had a live row for that exact key (`RULES.title,Rules`) — the CSV always wins when a row exists. Same trap as the CSV-driven-config family already documented above (dormant columns, render-time substitutions): **the code default is only ever a fallback for keys the CSV has no opinion on.** Before assuming a `uiStrings.ts` edit will reach the screen, `grep '^<KEY>,' public/data/CLEAN_FILES/UI_STRINGS.csv` — if a row exists, edit it too (and the local dev server's own writable copy, `server/data/game-data/CLEAN_FILES/UI_STRINGS.csv`, for immediate local verification without a restart). Caught only because the fix was verified live in the browser before being called done, not by trusting the code diff.

### Three reusable findings from the v3.2.71 planning + slice 1 session (2026-09-19)

**1. Raw "player" counts in `visitors.log` are mostly the nightly robot — filter before you conclude anything.** Since progress tracking went live (2026-08-03) there were 187 games, **149 of them the robot** (UA contains `Headless`/`Playwright`), 1 the maintainer's, and only **37 real outside people**. The unfiltered read said "outsiders stall at PM-DECISION-CHECK" — that was the robot's own stall. Recipe (read-only, prints aggregates only, no IPs): `ssh -o BatchMode=yes unraid "cat /mnt/user/appdata/Game_alpha/server/data/visitors.log" | node --input-type=module -e '…'`, importing `parseLogLine` from `server/visitorStats.js`; group by `gameId`; take origin and UA from the game's own `GAME_STARTED`; exclude robot UAs and the home IP; then count distinct `PLAYTEST_SPACE_REACHED` spaces. Baselines from that run are in TODO's Onboarding Phase C item. Small numbers: a pointer, not proof, and it cannot say *why* anyone left.

**2. A hand-built mirror of a panel needs a real-browser look — its tests can pass while the picture is wrong.** The editor's `PlayerPreviewPanel` is a separate renderer (its own header warns it drifts). Its story region is itself a `<button>`, so the new "?" had to be **overlaid as a sibling**, not nested — and an overlay takes no layout room, so it landed **on top of the first line of story text**. Every structural test passed; only opening the editor in real Chromium showed it. The fix reserves the same 32px title row the player panel has in flow, now pinned by a test. Whenever a control is overlaid rather than in flow, check what it covers.

**3. A page that outlived a server restart lies, and the screenshot tool has a fence.** After restarting Express under an already-open tab, a JS `.click()` "did nothing" and the next real click seemed to undo it — the page had a dead WebSocket. Reload first, then use explicit ≥ 1s waits. Separately, `mcp__playwright__browser_take_screenshot` only writes under the repo's gitignored `.playwright-mcp\` (a relative filename resolves against the repo root — don't); read the PNG back with `Read`, and `SendUserFile` it to Tom.

### Four reusable findings from the v3.2.69–70 session (2026-09-19)

**1. A design doc's count of "the one special case" is a hypothesis — measure the real data before writing the wiring.** The approved tooltip draft said exactly one dice button is merged (Con-Initiation). Ten lines of Python grouping the real manual dice rows by (space, visit) showed **8 combinations in four category pairs** (quality+bid, investor+time, work+team member, time+fee). The right design — `collapsePairedDiceActions` records `mergedFrom`, the "?" composes each merged category's approved text — only became visible after that count; wiring for one case would have explained half of what seven other buttons do. Sibling of "relayed briefs are hypotheses" (2026-09-10).

**2. `tests/vitest.setup.ts` resets module singletons after EVERY test — a `beforeAll` load leaves only the first test with data.** It calls `resetTooltipService()` (and other `reset…()` helpers) in a global `afterEach`. First run of the new guard: 31 of 62 failed, none for a product reason, and the one that passed was the only one that ran before the reset. Load in `beforeEach`. Before trusting file-level setup for any singleton, read `tests/vitest.setup.ts`.

**3. A "when X becomes true, open it once" state needs its previous value reset whenever the surrounding context changes.** The destination picker opens on the *transition* to "choosing is all that's left" (edge-triggered, so a player who folds it stays folded). Without `setPickWasOnlyGate(false)` in the space-change reset, "pick-only → player folds → arrives at another pick-only space" reads as no change and the new space stays folded. One sabotage isolates exactly that line. Also: the old test fixture hard-coded `movementChoiceUnlocked: true` beside an outstanding action — a state the engine never produces — so the rule reads both the flag and the panel's own arithmetic, and the fixture is now an option.

**4. Tooling: `cmd &` inside a command that already has `run_in_background` fires the "completed" notice when the wrapper subshell exits, not when the work does.** It produced a false "suite finished" read (the log had no summary line). Run the command itself with `run_in_background`, redirect to a file, append a `DONE exit=$?` marker, and wait on the marker. Pairs with "never pipe a backgrounded suite through `tail`".

### Five reusable findings from the v3.2.66–67 session (2026-09-18)

**1. A "whole panel vanished, no error, no console message" report can be the game-over screen, and it reproduces in ten minutes.** The 2026-08-15 "Con-Initiation Determine Outcome crashes everything" item sat open five weeks because every earlier attempt ran in a backgrounded tab. What settled it: build a *legitimate* state with the real services (`bootstrapHeadlessServices()` from `tests/ghost/bootstrapServices.ts`: add a player, `drawCards`, `updatePlayer({currentSpace, visitType, money})`, `startTurn`, `JSON.stringify(getGameState())`), swap in the browser's own player id, and `POST /api/games/:id/state` with `{state, clientVersion}` + `X-Game-Token` (token in `server/data/games.json`) — safer than hand-mutating fields, and the panel gates (`requiredActions` etc.) come out correct. Then run a **control**: same state, more money. $400K → the contractor price ($760K) exceeded cash → bankruptcy end screen replaced the board; $2M → outcome modal, panel intact. **A symptom list that matches a legitimate ending (`isGameOver`, `document.body.innerText` shrinking to the end screen, no `ErrorBoundary`) is that ending until proven otherwise.**

**2. The built-in Browser pane fires ZERO `requestAnimationFrame` frames (measured: `rafFramesIn1s: 0`, `visibilityState` flipping hidden/visible), so any Framer Motion enter animation sits at `opacity: 0` and looks like a broken modal — the Playwright MCP's own Chromium runs real frames (31 per 500 ms).** For enter/exit/animation checks use `mcp__playwright__*` (`page.evaluate` a click → poll `getComputedStyle(dialog).opacity` → click `button[aria-label="Close modal"]` → poll for removal); it writes to `.playwright-mcp/`, which is gitignored. Reading `opacity: 0` in the Browser pane is evidence of the pane, not of the code — check rAF first (`await new Promise(r=>{let n=0;const t=performance.now();const f=()=>{n++;performance.now()-t>500?r(n):requestAnimationFrame(f)};requestAnimationFrame(f)})`).

**3. A pipeline's default branch can turn text it does not recognise into a real action that does the OPPOSITE of what the label and story say.** Four "Pass help to your left/right" buttons gave the pressing player a free expeditor. `processGameData.js` only recognises `^(Return|Replace|Give|Draw)\s+\d+`; the legacy sentence "The person to your right takes a card." fell through to the default `draw_E`, count parsed as 1 by `parseInt(...) || 1`. The engine's `transfer` action (`CardEffectService.handleTransferCard`, condition `left|right`) had existed for months with no producer. Reading the label, the story and the engine each looked right; **only running the real row showed it** — a 20-line throwaway probe (`bootstrapHeadlessServices`, count `getPlayerCards(id,'E')` for both players before/after `triggerManualEffect`). Same family as "Card bare-number type default = W". When an effect is authored as free text, probe what the engine does with the actual row and compare it with the label. Details and options: TODO 🙋.

**4. The `Edit` tool rewrites a CRLF file to LF wholesale** — a 40-line change to `docs/user/USER_MANUAL.md` became a 774-line diff. Check `git diff --stat` after the first edit to any file; if it is far larger than the edit, restore with `sed -i 's/$/\r/' <file>`. (Companion to the Python line-endings entry: the cause differs, the symptom and the fix are the same.)

**5. TypeScript 6 is a real migration, and 7 is not available to this toolchain.** `moduleResolution: "node"` and `baseUrl` are deprecated errors — migrate to `"bundler"` and `paths: {"@/*": ["./src/*"]}`, never `ignoreDeprecations`. Two latent problems surface: a side-effect import of a missing module is now an error (the orphan Jest `tests/setup.ts` importing `jest-environment-jsdom`), and bundler resolution respects package `exports`, so importing an *undeclared transitive* dependency whose `exports` has no `types` (`dom-accessibility-api@0.5.16`) fails — declare it (`^0.7.1` ships types). `typescript` is pinned `~6.0.3`: `@typescript-eslint` 8.70 peers `<6.1.0`, while `latest` is 7.0.2. And a drift guard is only proven by **sabotage**: revert one default, watch the new test name the key, restore.

### html2canvas clips a `position: fixed` element to an `overflow: hidden` ANCESTOR — a browser does not (2026-09-17, v3.2.61)

Bug-report screenshots (fb:f33ae50b) lost some modals: the capture showed the player panel dimmed and no dialog. Only modals opened from INSIDE the player panel were affected. ModalBase renders its fixed overlay as a DOM descendant of whatever opened it, and the panel card is `overflow: hidden`. A real browser ignores that ancestor for a fixed element; html2canvas applies it, so the backdrop was painted only inside the panel's rectangle and the centred dialog was cut away. The tell is a dim confined to exactly one container's box: measure the nearest `overflow != visible` ancestor's rect and compare. The fix belongs on the CLONE (`onclone`: move each `[aria-modal]`'s outermost fixed layer to `<body>`), which cannot change what a player sees. Portalling every modal would also fix it, but it touches every modal and every test that queries a render container.

### Vite's watcher can miss an edit — prove the served module before trusting a live check (2026-09-17, v3.2.61)

A live check said a new hover handler did nothing. The source was correct: `curl http://localhost:3000/src/<file>` returned the OLD module, because the dev server's file watcher had missed a write (files rewritten by a Python patch script on D:). Other files edited the same minute were picked up, so "some changes are live" proved nothing about this one. Before debugging a live check that contradicts the source: `curl -s localhost:3000/src/<path> | grep -c <new-identifier>`. If it's 0, `touch` the file, re-curl, and reload. Reading `__reactProps$…` on the DOM node (a missing `onMouseEnter`) is the in-page version of the same check.

### A drifted second copy of a rule is fixed by DELETING it — and measure the data before changing any default (2026-09-10)

Workstream 6 audit II found the same CSV `condition` string answered **false** on the arrival path and **true** on the manual-button path. There were four evaluators, not the two reported. A private NPC-name table said "The Banker" while CHARACTERS.csv said "The Bank". A component kept its own phase table. The fix for each was **delete the copy and read the one source**. Re-syncing two copies only restarts the drift on a slower clock. Three habits that made it safe (all detailed in CHANGELOG v3.2.56):
- **Before flipping a default, enumerate what reaches it.** A default of "unknown → true" may be carrying a live effect today; changing it to "false" would then be a silent regression. List every distinct value across the repo, the server and the **live** classroom copy (ssh `docker exec game_alpha cat …`), and record which code path reads each value. Here only `dice_roll_1..6` existed, all on auto rows, so the divergence was latent. Find that out first, then unify.
- **A CSV column can be a gate AND a parameter.** SPACE_EFFECTS `condition` holds both `dice_roll_3` (does this apply?) and `per_200k` / `to_left` (how much? to whom?). One evaluator must treat the parameter forms as "run it", or the handler that reads them never gets called.
- **Prove a regression test catches the bug by running it against the old code.** `git show HEAD:<file> > <file>`, run the test, restore. Every test added here failed on the old code (7 of 11, 2 of 5, 4 of 4, 1 of 6). Separately, the pre-existing reskin tests varied only the card **ID** axis and never the card **TYPE** axis, which is exactly where the `card_type === 'E'` leak lived. A suite named after a hazard is a claim; its assertions are the evidence.

**Relayed briefs are hypotheses, whatever they say about themselves.** The brief for this work said its file:lines were "verified facts". Four of its claims were wrong: a "blocker" key that no code path can reach, a classroom copy called a "frozen snapshot" when it re-bakes itself on a stock-hash change, derived data copies treated as independent sources, and the evaluator count. Each was settled in minutes by reading the code or trying it locally. Check before you act on, or repeat, a claim someone else passed along.

### A Playwright click that times out is never an inert button — and the interceptor is a property of the PAGE, not the control (2026-09-09)

The nightly robot reported 18 clicks that "would not land": `is_pressable()` passed and then `ElementHandle.click: Timeout 8000ms exceeded`. That reads as a dead control and is not one. **Two facts settle it without sampling.** First, a `.click()` timeout is raised by the actionability wait **before the event is dispatched**, so a button whose handler no-ops reports *success*, never a timeout — "genuinely inert" is excluded by the error class alone, with zero extra runs. Second, Playwright prints which of its four conditions (stable / visible / enabled / receives-events) failed, in its own call log: `element is visible, enabled and stable` … `<div data-testid="dice-result-modal-overlay">…</div> intercepts pointer events`. **Read the call log; do not infer the cause from which buttons failed.**

The mechanism is [`ModalBase`](src/components/modals/shared/ModalBase.tsx)'s backdrop — `position:fixed`, `inset:0`, `z-index:1000`, and it also sets `body.overflow='hidden'`. Because it covers the viewport, **every** control behind it fails identically: action button, commit button, destination row. `is_visible()`/`is_enabled()` are properties of the *element* and are both still `true` — being covered changes neither, so no attribute a harness can read says "something is on top of me". Only `elementFromPoint` or a trial click can tell you. Every `ModalBase` modal already exposes copy-independent handles for exactly this: `[data-testid="<name>-overlay"]`, `[data-testid="<name>"]`, `role="dialog"`, and `button[aria-label="Close modal"]`. **One exception:** [`RoutingExplanationModal`](src/components/modals/RoutingExplanationModal.tsx) is not a `ModalBase` — it has `role="dialog"` but no testid, no `aria-label="Close modal"`, and deliberately no backdrop-close; its only exit is a button reading "Got it", so it is still copy-coupled.

**The tell, and it is the expensive half:** this single failure was named after the wrong *shared attribute* four times in one day — the space, then the button kind, then one effect at several spaces, then `replace_e` — and each correction was as under-evidenced as the claim it replaced. Every one was an honest summary of the visible data; none felt like guessing. The mechanism survived all four **because it was read off an instrument instead of inferred from a pattern across cases.** Sibling of the copy-coupling entry below: when the robot looks stuck, prove what it can and cannot *reach* before touching game code (v3.2.53 shipped against this symptom).

### A zero from a grep is a claim about your PATTERN before it is a claim about the artifact (2026-09-08)

Verifying v3.2.54's deploy, a content check for `Pick where you're going first` returned **0** against a bundle that contained it, and nearly became a written-up failed deploy. The string uses a **typographic apostrophe** — 3 bytes in UTF-8 — so a straight-quote `'` grep misses it, and so does the reflexive `you.re` fallback, because `.` matches one *byte* of a three-byte character. Either paste the exact character (`grep $'you’re'` or the literal) or match around it (`grep "Pick where you"`). This is the same false-failure family as the v3.2.47 precedent and as the code-splitting trap already recorded in the deploy-verification step: **before reporting an absence, prove the pattern can find the thing when it IS present** — grep the local `dist/` copy first, which the deploy-verification recipe already tells you to do for a different reason. Apostrophes, en-dashes, ellipses and non-breaking spaces in authored player copy are all this hazard; the game's voice work produces them constantly.

### A rule written by hand in N places will eventually be missed in the N+1th (2026-09-07, v3.2.53)

The skippable-action rule — `replace_/return_/give_` are optional and must not gate the turn — existed as a hand-written regex in `StateService.calculateRequiredActions` and twice in `ManualActionProcessor`. It was obeyed in all three and **missed in the fourth place that needed it**: PlayerPanelV2 built its "what is blocking you" line from every uncompleted manual action, skippable ones included. The result was a commit spine reading *Finish "Swap one helper for another" above first* on a space where the swap was optional and the real requirement was picking a destination — an instruction that could not be followed, on all 12 choice-movement spaces. Fixed by extracting [`isSkippableEffectAction`](src/utils/skippableActions.ts) and routing all four through it. **The tell:** a domain rule expressed as a literal regex or prefix list in more than one file is a latent bug, and the missing copy is usually in the *presentation* layer, because that's the file nobody thinks of as enforcing rules. Grep for the rule's shape (`/^(replace_|`), not for its callers, before adding a new consumer of the same concept.

### A branch ordered after a broader condition can be permanently unreachable — check the guard, not just the string (2026-09-07, v3.2.53)

`'Pick where you're going first'` sat in the commit-spine `subLabel` chain behind `remaining > 0`. Choice movement contributes +1 to `requiredActions` and stays uncompleted until `moveIntent` is set, so on **every** choice-movement space `remaining` was at least 1 whenever no destination was picked — exactly the condition the later branch existed to describe. The string was in the source, read fine in review, and could never render. When a fallback branch describes a *specific* state, verify that state isn't already subsumed by an earlier, broader test: decompose the counter (here, subtracting the movement pick out of `remaining`) rather than ordering conditions and hoping. Grepping for a user-facing string proves it exists, never that it is reachable.

### `TextWithTerms` cannot live inside a `<button>` — put the explanation beside the control, not on it (2026-09-07, v3.2.54)

`TextWithTerms` renders a glossary term as `<span role="button">` with `stopPropagation()`, so nesting one inside a real `<button>` swallows the press. This is why v3.2.51 stripped trade words from action buttons — and why the teaching layer's explanation is a **sibling** of the action button, never a child. If a hard word must be tappable near a control, the word goes in adjacent markup. Related trap: `onTermClick` is an **optional** prop — omit it and terms render underlined and do nothing, which looks like working markup and isn't. The panel's established call is `onTermClick={(term) => openWithTerm(term.id)}`.

### The glossary silently matched no plurals for the life of the feature (2026-09-07, v3.2.54)

The highlighter builds its regex from the word index with `\b...\b` boundaries, so `Expeditor` could not match "Expeditors", and none of the 249 authored terms carried its own plural as an alias. Construction copy is made of plurals, so the teaching surface was dark exactly where it reads most naturally — 25 distinct plurals in the shipped copy went unlinked. Fixed by generating regular plurals **into the existing word index** rather than loosening the regex, so the matcher, its boundaries and `findTermByWord` stay untouched and a generated form can never shadow an authored one. **General shape worth keeping:** when a text-matching feature under-fires, widen the *index*, not the *pattern* — the index is measurable (257 generated, 0 collisions, 25 live hits in shipped copy) and a loosened pattern is not. Also: glossary data can contain its own CSV header pasted back as a row (`term="term"`, `definition="definition"`, which made the word "term" open a definition reading "definition"); guard in the parser, not the data, because the nightly scraper regenerates the file.
...
|---|---|---|
| **Authored button label (wins over everything below)** | `SPACE_EFFECTS.button_label`, from `Spaces.csv`'s `w_/b_/i_/e_card_label` + `DiceRoll Info.csv`'s `button_label` | **SOURCE CSVs, then `node scripts/regen-clean-files.mjs`** — all 80 manual rows authored in v3.2.51 |
| Manual card-action button label ("Get Bank Loan") — now only a FALLBACK for unauthored rows | `formatManualEffectButton` card branch | `src/utils/buttonFormatting.ts` |
...
**One speaker map (v3.2.56).** The dice summary's speaker name comes from `getNpcCharacterInfo(spaceName).name` — i.e. `CHARACTER_MAP`, populated from `CHARACTERS.csv` — like every other surface. `DiceService` used to keep its own `NPC_SPEAKER_NAMES` table "by design"; it had drifted ("The Banker" there, "The Bank" everywhere else) and was deleted in the Workstream 6 audit II. When adding a new NPC: add a `CHARACTERS.csv` row (clean-only file — no SOURCE copy, no pipeline step; portrait art is optional, `image_roles` may be empty) and, if the space name doesn't follow the prefix convention, point it at the new id with GAME_CONFIG's `npc_speaker`. `extractPrefix()` in `characters.ts` handles the `REG-DOB` / `REG-FDNY` two-segment exception — any new multi-segment prefix needs the same handling (or an `npc_speaker` override).
...
- **Windows can't atomic-swap the `resolved/` dir while the server serves it** (EPERM on `rename`). An external re-bake — or a game-create re-bake when `configVersion > resolvedVersion` — 503s with `Bake failed at game create: EPERM`. **Stop the server → bake → restart.** Boot-time bake works (handle not yet held); Linux/prod renames open dirs fine, so this is Windows-dev-only. **Sharpened + partly fixed 2026-08-22:** the Express server was never the main culprit — **Vite's file watcher was**. `vite.config.ts` had no `server.watch.ignored`, so chokidar watched `server/data/**` (which is not part of the frontend build at all) and held handles on `instances/*/resolved/`. That made EPERM fire on **every** instance mutation while `npm run dev` was up, not just on a concurrent offline bake — isolated by controlled test (Express alone always succeeds; adding Vite always fails). Fixed by scoping the watcher (`ignored: ['**/server/data/**']`). If EPERM ever returns, check that setting first, then fall back to the stop-server-bake-restart dance above.
...
**Correction and completion (2026-09-01, v3.2.44): "renders at a reduced resolution" was the wrong mental model, and it cost two sessions.** The TV is not rendering at 960×540 and upscaling — nothing is blurry. `screen.width × devicePixelRatio` = 960 × 4 = **3840**: every glyph is drawn at four device pixels per layout pixel, sharper than any desktop. What is small is the **layout budget**, not the picture. The distinction matters because it changes the fix entirely — "the TV renders low-res" suggests nothing can be done, while "the app only asked for 960 layout px" is a one-line viewport-meta change.

Three practical consequences:

1. **`devicePixelRatio` is the missing number, and it is free.** Nothing in this repo read it before v3.2.44. A user reporting "the resolution is very low as if on a phone" on a 4K set is describing layout density, and only the ratio distinguishes that from a genuinely small screen. Bug reports now carry `devicePixelRatio`/`screenPixels`/`physicalPixels` (`FeedbackButton.tsx`) — read them before theorising. The Android UA string also embeds the device model verbatim (`SmartTV 4K FFM`), which is a second free signal.
2. **Widening the viewport buys room and costs legibility, proportionally.** `<meta name="viewport" content="width=1280">` on a 960 device gives 78% more layout area and shrinks everything to 3/4 size. Going to true 4K (3840) gives 15× the area at 0.13° of visual angle — unreadable at 10ft. Worked table and geometry in `src/utils/tvScale.ts`. **Never treat "make it 4K" as a straight win.**
3. **The remaining unknown is not measurable, so stop trying to infer it.** Physical screen size and viewing distance decide the right text size, and a 32" monitor at arm's length reports width, ratio and screen identically to a 75" TV across a classroom. There is no API. `tvScale.ts` asks the viewer instead, with samples rendered at the size each choice actually produces — the maintainer's own idea. Reach for that pattern rather than a device-model lookup table: the lookup table is maintenance forever for data the browser already volunteers, and it still would not answer the distance question.

### A summary endpoint that projects a subset of a record loses fields silently — nothing ever fails (2026-09-01)

`GET /api/public/feedback/open` builds its response by hand-listing fields off each stored report (`whatDoing`, `whatWrong`, `version`, …). It never listed `extra` — the free-text box where a reporter writes the part that does not fit on one line. The form captured it, the file on disk stored it, and the only view anyone triaged from dropped it. **No error, no warning, no empty render: a response object quietly missing a key is indistinguishable from one that never had it.** It survived months and let fb:93449bf2 be fixed and closed on its one-line summary while its substantive paragraph — a correct diagnosis of a real problem — went unread. It surfaced only because the maintainer happened to remember writing it.

Cost, measured once the field was restored: **42 of 222 reports carry `extra` text, 4 of them still open**, including a second, entirely separate feature request buried inside a report that had been investigated since July on its first line alone.

Generalises to any hand-built projection in this repo (dashboard summaries, `consoleSummary`-style digests, admin list views): **when you narrow a record for a cheaper view, the omission is the bug you will never notice.** Two cheap defences, both now in place here — pin the projected shape with a test (`tests/server/feedbackTriageShape.test.ts`, source-fingerprint style since `server.js` auto-listens and cannot be imported), and prefer truncating a long field over omitting it, the way `consoleSummary` already did.
...
### When the Browser pane isn't displayed, React Flow's edges render zero DOM output — verify "did I break this" with a git-revert A/B, not by reading the code again (2026-08-02)

Building G160 (per-edge waypoint redirect), the board's connector lines showed **zero** `.react-flow__edge` DOM elements in this tool's Browser pane after the change — even a component body hard-coded to `return <circle fill="red"/>` produced nothing. Before concluding the new edge component was broken, `computer{action:"screenshot"}` was tried and returned the exact error this file already documents elsewhere: *"the Browser pane is not displayed, so the page is not compositing frames."* That's the same root cause as the AnimatePresence-won't-dismiss entry above, but a different, less obvious symptom — there it was an animation callback never firing; here entire node output silently never reached the DOM at all (React Flow's edge layer apparently depends on real layout/compositing to mount anything, not just to animate it).

**The diagnostic move that settled it for certain, not just plausibly:** `git diff <file> > /tmp/patch`, `git checkout -- <file>` (reverting to the last commit, before any of the session's edits), reload, and check whether the SAME symptom persists on code that is definitely unmodified. It did — zero edges on the pristine baseline too — which is conclusive proof the pane, not the new code, is the cause. Then `git apply /tmp/patch` restored the in-progress work. This is a general technique, not board-specific: **whenever "is this a real regression or a tool/environment artifact" is in doubt and the file has a clean last-committed state to compare against, a revert-reload-restore round trip is faster and more certain than re-reading the diff for the tenth time.** Costs one git stash-equivalent cycle; settles the question with evidence instead of a guess.

**Practical fallout:** don't burn time chasing "why don't my edges render" in this tool when the pane isn't visibly displayed — it's not fixable from the code side. Ship the change on the strength of typecheck/lint/build/tests + careful reading, flag the visual gap explicitly in the handoff, and get a real-browser confirmation before calling the feature fully done.
...
### G160 "hover works, drag doesn't" traced to the wrong tool being tested, not a CSS/JS bug — and console.log is a bad diagnostic channel when a human is relaying output by hand (2026-08-04)

A multi-round debugging session chased a React Flow custom-edge drag handle that showed a grab cursor but wouldn't actually drag. First guess was that the handle needed `className="nodrag nopan"` to stop React Flow's pane-pan from stealing the mousedown — shipped as a fix, user reported no change. Reading `@xyflow/system`'s actual source showed why: `noPanClassName` (`'nopan'`) is already applied to every edge wrapper `<g>` by default, so the explicit class was a no-op guess, not a fix. The real cause, found several rounds later: the user had been testing the drag in `BoardLayoutEditor.tsx`, a separate standalone admin tool that was never wired to the waypoint setter props at all (`grep` showed it passed only `isAdmin={true}` to `<BoardCanvas>`, no `onSetEdgeWaypoints`/`edgeWaypoints`) — it looked identical to the in-game admin toggle that *was* wired up. Every diagnostic log had actually been showing `isAdmin:true, hasSetter:false` the whole time; the fix was recognizing that split as "wrong surface," not chasing more CSS theories. **Two lessons compound here:** (1) when a drag/click handler "looks wired but does nothing," check whether the user is even exercising the code path you're editing — grep for what props actually reach the component under test, don't assume the screenshot's tool is the one you last touched. (2) console.log is unreliable once a human is relaying results by copy-paste instead of reading DevTools live — expanded object args collapse to bare `Object` text on copy, and a log line can be scrolled past in a wall of re-render noise. Switched first to `JSON.stringify`-flattened single-string log lines, then to an on-screen fixed-position visible badge (CustomEvent-driven) once even flattened logs kept getting missed — prefer the on-screen badge from the start whenever the user is hand-relaying output rather than watching DevTools themselves.

### The shared CSV parser can't round-trip a literal `"` inside a field — plan around it, don't patch it (2026-08-14)

Building `UI_STRINGS.csv` (the button/notification-text reskin lift), one string (`NOTIF.cardPlayDetailed`, `{player} activated "{name}" with effects: {summary}`) embeds literal double-quote characters. `DataService.parseCsvLine` — shared by every CSV loader in this codebase — toggles `inQuotes` on *every* `"` it sees and never emits the character, so it has no RFC4180 `""`-escaping support: a doubled quote just flips the state twice and both quote characters vanish from the parsed value, silently corrupting the field rather than erroring. Fixing the parser itself is out of scope for a one-string problem — it's load-bearing for every other CSV loader in the app and a change there needs its own full audit. The actual fix: leave that one key out of the CSV entirely (it stays an in-code-only default in `uiStrings.ts`, routed through the same `getUIString()` lookup for consistency, just never overridable via the file) and document why, right at the type definition (`UIStringCsvRow` in `DataTypes.ts`) so a future session doesn't reintroduce the same corruption trying to "complete" the CSV. If a future reskin genuinely needs to override quoted text, use a Unicode smart-quote (`" "`) in the CSV value instead of straight ASCII quotes — sidesteps the parser limitation with a barely-visible cosmetic difference, cheaper than patching the shared parser.

### Testing a React component whose behavior depends on mutable module-level config state: a test-only reset export beats `vi.resetModules()` (2026-08-14)

`uiStrings.ts`'s `configureUIStrings()` mutates a module-level `Map` that every test in a file shares (isolate:true only resets state *between test files*, not between `it()` blocks in the same file — see `vitest.config.ts`). The instinct from other CSV-driven-config regression tests in this repo (`CharacterMapCsvDriven.test.ts`, `ViolationRulesCsvDriven.test.ts`) is `vi.resetModules()` + a dynamic `await import(...)` per test for full isolation — safe for those, since they only call plain functions. It's a trap for a test that **renders a React component** pulling from the same config module: dynamically re-importing after `resetModules()` also re-imports React itself fresh, and `@testing-library/react`'s `render()` still holds the *original* (statically-imported) React/ReactDOM instance — a recipe for a duplicate-React-instance / invalid-hook-call class of failure. `RulesModal.test.tsx` uses static imports throughout instead, plus a small test-only export added to `uiStrings.ts` — `_testOnly.resetUIStringOverrides()` — called in `afterEach` to clear the override map between tests (mirrors `endGameInsights.ts`'s existing `_testOnly = { rules }` pattern for exposing test-only internals). Rule of thumb: reach for `resetModules()`+dynamic-import only when the test never touches JSX/React; for anything that renders, add a deliberate reset hook to the module instead.

### Enabling real IPv6 on a Docker/Unraid host needs a `forwarding`+`accept_ra=2` fix the GUI never sets — and the GUI can silently disagree with the actual running service (2026-08-16)

Root-causing the long-standing IPv6 false-foreign-alert bug (server can't detect its own home IPv6 → every real IPv6 visitor from home reads as foreign) led to actually turning IPv6 on for Tower, live, with the user driving each host-level step. Three traps, all specific to a bonded+bridged Docker host, not IPv6 in general:

1. **The real interface chain is `eth0 → bond0 → br0`, and IPv6 must be enabled on all three, or a check on just one gives a false "no IPv6 here" reading.** Unraid's "IPv4 only" GUI setting sets `net.ipv6.conf.<iface>.disable_ipv6=1` independently on `eth0`, `bond0`, AND `br0` — not just one shared toggle. The host's real LAN address (`192.168.86.57`, DHCP) lives on `br0`; testing `eth0` alone (flipping only its `disable_ipv6`) produces nothing but a link-local (`fe80::`) address and looks like "no IPv6 available" even when the ISP/router genuinely offers it.
2. **Even with `disable_ipv6=0` on the whole chain, a Docker/VM host still won't pick up a real SLAAC address — because `net.ipv6.conf.br0.forwarding=1` (needed for Docker/libvirt routing) silently changes what `accept_ra` means.** Linux's `accept_ra` default (`1`) means "accept Router Advertisements only if forwarding is OFF for this interface." A host that routes for containers/VMs has forwarding ON, so RAs are accepted-in-name-only and thrown away — only a link-local address ever appears, identical symptom to trap 1, easy to misdiagnose as "still not working" after supposedly fixing trap 1. Fix: `net.ipv6.conf.br0.accept_ra=2` (overrule forwarding, accept anyway) — not exposed anywhere in Unraid's network GUI, has to be set via terminal and persisted in `/boot/config/go` (`sysctl -w net.ipv6.conf.br0.accept_ra=2` appended) for it to survive a reboot.
3. **A service's GUI toggle can show "No" while the actual persisted config and the actual running process both say otherwise — trust `ps aux`/the config file over the display.** Unraid's VM Manager page showed "Enable VMs: No" (this box has no VT-x/AMD-V, so it's normally off) and clicking Apply visibly did nothing — the toggle reverted to "Yes" on navigating away. `cat /boot/config/domain.cfg` showed `SERVICE="enable"` the whole time, and `ps aux | grep libvirt` showed `libvirtd`/`virtlockd`/`virtlogd`/`dnsmasq` all genuinely running — the GUI was simply not reflecting reality. Fix was direct: `/etc/rc.d/rc.libvirt stop` (bypasses the GUI entirely, stops the real service). **When a GUI toggle "doesn't stick," verify against the actual process list or on-disk config before assuming user error or re-clicking — the display and the reality can just be out of sync.**
4. **Unraid's own `rc.docker` startup script already passes `--ipv6=true --fixed-cidr-v6=fd17::/64` as command-line flags to dockerd — also setting those same two keys in `/etc/docker/daemon.json` doesn't merge, it crashes dockerd outright.** Discovered finishing the Docker-side half of this same fix later the same day: `/etc/rc.d/rc.docker restart` failed with `rc.docker: Docker daemon... Failed`, taking every container on the box down (docker.sock unreachable) until rolled back to a pre-edit backup. `rc.docker`'s own console output only says "Failed" — `/var/log/docker.log` has the real reason: `unable to configure the Docker daemon with file /etc/docker/daemon.json: the following directives are specified both as a flag and in the configuration file: fixed-cidr-v6:..., ipv6:...`. **Always `cp` a config file before editing it** — recovery was one `cp` back, not hand-reconstructing content. Once the conflict was found, no `daemon.json` edit was needed at all: the daemon already runs its own IPv6 NAT for the default bridge, so a *custom* network (`game-net`) just needed its own `docker network create --ipv6 --subnet <ULA>` — smaller, lower-blast-radius, and doesn't touch daemon config.

**Also confirmed: this session's safety classifier hard-blocks live `sysctl -w` network changes on the remote host even with explicit in-chat user permission ("go ahead").** Not a one-time prompt to work around — every retry of the same class of command was blocked identically. The correct move (per the tool's own guidance) is to stop retrying and hand the exact command to the user to run themselves via SSH/the Unraid terminal, then verify the *result* read-only afterward. This applied to every host-network-affecting step above; the user ran all of them directly, this session only diagnosed and verified in between.

### A container stuck immediately failing its own auth check may have a literal unfilled placeholder, not an expired credential — check `docker inspect`'s actual `Cmd` before trusting the dashboard (2026-08-16)

`cloudflare-tunnel` was crash-looping (`docker ps` showed `Restarting (255)`), logs read `Provided Tunnel token is not valid.` — the natural read is an expired/revoked token, and Cloudflare's own dashboard reinforced that guess (Zero Trust > Tunnels listed zero tunnels, consistent with "it got deleted"). `docker inspect cloudflare-tunnel --format '{{.Config.Cmd}}'` showed the actual answer instantly: the container's own startup command was still `tunnel --no-autoupdate run --token YOUR_TOKEN_HERE` — a literal, never-filled-in placeholder from however this container was originally set up, undetected for at least the 12 days `docker ps`'s CREATED field showed, since nothing else depended on outside reachability being checked daily. No amount of looking at the Cloudflare dashboard would have found this — there was never a real tunnel to find there. **When a container's own auth/token check fails consistently (not intermittently) and the service-side dashboard shows nothing wrong, check what the container was actually told to run before assuming the credential itself went bad** — `docker inspect <name> --format '{{.Config.Cmd}}'` first; if checking `.Config.Env` too, mask values before printing them (a real secret, unlike a placeholder, shouldn't get echoed into a terminal/log unnecessarily).

### Reading `globalActionLog` for "what happened THIS turn": the current turn's own entries are deliberately `isCommitted: false`, and turn-1 entries are stamped `globalTurnNumber: 1` while `globalTurnCount` is still `0` (2026-08-19)

Building the End Turn cost-preview's "show the real result instead of going blank" fix (`costPreview.ts`'s `findTurnCardDrawFragment`, closing a maintainer report), needed to read `state.globalActionLog` for "what did this player actually draw earlier THIS turn." First attempt reused `getDisplayableLogEntries()` — the same helper `PlayerChronicleV2`'s post-hoc history view already uses — which filters `!entry.isCommitted`. Live-testing found it always returned zero rows for the turn in progress; `GET /api/games/:id/state` showed why: a card drawn earlier the same turn is `isCommitted: false` in the log, correctly, since it's still TEMP state that only gets promoted to real/committed history once End Turn actually resolves it (the whole point of the REAL/TEMP architecture behind Try Again). `getDisplayableLogEntries`'s `isCommitted` filter is right for the Chronicle (only show finalized history) and wrong for anything that needs to see the in-progress turn's own actions — read `globalActionLog` directly instead, filtering by playerId/type/turn number but deliberately NOT `isCommitted`.

Second, separate trap found the same session: on a game's very first turn, `state.globalTurnCount` is still `0` (not yet incremented by `advanceTurn()`), but `LoggingService.ts` stamps every entry logged that turn with `globalTurnNumber: gameState.globalTurnCount || 1` — the `|| 1` fallback means turn-1 entries carry `globalTurnNumber: 1`, not `0`. A naive `entry.globalTurnNumber === state.globalTurnCount` filter compares `1 === 0` and silently misses every first-turn entry — confirmed live (a real Work Package draw on turn 1 produced no preview row until this was fixed). Match the same `state.globalTurnCount || 1` fallback anywhere "this turn's log entries" needs computing.

### The Browser pane never holds keyboard focus — `.focus()` changes `activeElement` but fires NO focus events (2026-08-23)

Building the editor's "clicking into a field shows you what it changes" link (v3.2.37), the feature refused to work in the pane: focusing a field left the player view dark. It was not the code. `document.hasFocus()` returns **false** in the pane, and a browser that does not have document focus dispatches **no `focus`/`focusin` events at all** — measured directly by attaching a listener to the element and calling `.focus()` on it: `activeElement` moved, the listener fired **zero** times. Everything downstream of a focus event is therefore invisible to this environment, which is also the real reason the v3.2.34 Tab-order check could never be run (recorded there as "synthetic keystrokes do not reach the page" — same wall, now root-caused).

What does work: dispatch the event the browser would have sent, `el.dispatchEvent(new FocusEvent('focusin', {bubbles: true}))`. React listens for `focusin` at the root container, so this drives the identical handler path as a real click — genuine verification of the wiring, though still not proof that a human clicking produces a focus event on their machine. Say which one you did. Two traps while doing it: read the resulting DOM **after a `setTimeout` tick**, because reading synchronously after dispatch returns the pre-render DOM and looks exactly like a broken feature; and `focusout`'s `relatedTarget` must be set via `Object.defineProperty` since the constructor ignores it.

Same family as the hidden-tab animation artifacts already documented above: before concluding an interaction feature is broken here, check whether the pane can generate the input at all.

### Patch scripts: preserve the file's line endings, and never inline one containing backticks into bash (2026-08-23)

Two separate self-inflicted wounds from the same session, both from driving edits through `python` from the Bash tool.

**Line endings.** `.gitattributes` says `* text=auto eol=lf`, but several files are stored **CRLF in HEAD anyway** (`SpaceEditor.tsx`, `PlayerPreviewPanel.tsx`, `SpaceDeckScreen.tsx`, `DataEditor.test.tsx` — `spaceRegions.ts` and most test files are LF). A script that reads with python's default universal-newline translation and writes with `newline=''` silently rewrites the whole file to LF: a ~500-line change came out as a **~2500-line diff**, burying the real edit. Read with `io.open(p, newline='')`, detect `'\r\n' in s`, and convert every old/new patch string to the file's own endings before matching. Check with `git diff --stat` before committing — a diff far larger than the edit is the tell.

**Backticks.** Passing a script to `python -c "..."` inside a double-quoted bash string lets **bash run any backticked text as a command first**. A script whose *prose* mentioned `` `npm ci` `` actually executed `npm ci`, reinstalling `node_modules` unasked, while the script itself died on a syntax error from the mangled quoting. Markdown-ish prose is full of backticks, so this fires constantly when writing docs or TODO items from a script. Write the script to a file (scratchpad) and run `python <file>` — never inline it.

### `io.open(path, 'w')` truncates BEFORE the write — a patch script that raises mid-write leaves the file EMPTY (2026-08-26)

Companion to the line-endings entry above, same tool, worse failure. A `/koniec` doc edit did `io.open(p,'w',encoding='utf-8').write(s)` where `s` contained an emoji built from a `📜` surrogate pair (valid in a Python string literal, **not** encodable as UTF-8). Python opened the file — **truncating it to zero bytes** — then raised `UnicodeEncodeError` on the write. `RELEASE_NOTES.md` (3435 lines) went to 0 and the error message said nothing about data loss. Recovered with `git checkout --` because it was committed; an uncommitted file would simply have been gone.

**Two rules:** (a) prefer the `Edit` tool over a Python round-trip for anything already committed and readable — it matches a string and cannot truncate; (b) if a script must write, build and validate the full output string **before** opening the file for writing (`s.encode('utf-8')` in a try, or write to a temp path and `os.replace`). Never let an open-for-write and a possibly-raising expression sit in the same statement.

**RECURRED TWICE on 2026-09-07 (v3.2.54) — despite this entry existing AND being restated in the `/koniec` handoff reminders.** It emptied `tests/E2E-01_HappyPath.test.tsx`, and then emptied **this file** in the very step that was documenting the first occurrence. Knowing the rule is demonstrably not enough; the pull toward a quick Python patch is strongest for a two-line edit.

**Now root-caused, and the cause is specific to this harness:** a `\uXXXX` escape written inside a Bash-tool heredoc can reach Python as a **real lone surrogate character** rather than as the six literal characters intended. Both failures carried emoji escapes (`🎯`, `⚡`) in a regex or replacement string; the resulting string is unencodable, so `write()` raises `UnicodeEncodeError: surrogates not allowed` **after** `open(p,'w')` has already truncated the file to zero. Verified after restoring: the file itself contained zero surrogates, so they could only have come from the script text. **So: never put `\uXXXX` escapes in a heredoc'd script** — match the literal character, or a `.` wildcard, instead.

Two diagnostics, because the failure is silent: a truncated test file reports **"no tests"** rather than a failure, and after any mid-write raise the sweep is `for f in $(git diff --name-only); do wc -c "$f"; done` — the error names the encoding problem and never the file it had already opened. Recovery is `git checkout -- <file>`, which also **discards uncommitted edits to that file**, so anything added earlier in the session must be redone.

Also: write emoji as `\U0001F4DC` (or the literal character), never as a `📜` surrogate pair — Python accepts the pair in source and then refuses to encode it, which is exactly the mid-write raise above.

### "The suite is green" depends on WHICH suite — the batch runner does not cover every test file (2026-08-26)

`tests/scripts/run-tests-batch-fixed.sh` is the documented pre-commit gate ("all 23 batches green") and it enumerates test files **explicitly**. It does not glob. A v3.2.41 change to `ClassroomSetup.tsx` broke all 10 tests in `tests/components/classroom/ClassroomSetup.test.tsx` — a directory the batch script never lists — so the change passed **22/22 batches** and the failure only surfaced at `/koniec`, from `npm test`, which runs all 198 files.

**Rule: `npm test` is the real gate; the batch runner is a fast subset.** When a change touches a component whose tests you have not seen run by name, run that file directly (`npx vitest run <path>`) rather than trusting a green batch count. And when adding a new test directory, add it to the batch script or it is invisible to every batch run forever. Same family as the "check what a quality metric MEASURES" entry above — a green number over an unstated subset is worth less than it looks.

### A permission boundary cannot be proven by unit tests — stand up a real server and try to break it (2026-08-26)

v3.2.41 opened `POST /api/instances/:id/content` to teachers (previously admin-only). Unit tests passed, source-scanning auth tests passed, the full suite was green — and none of that demonstrated the boundary actually holds, because every one of them exercised the *pure* restriction function or grepped the route's source text, not the running route with a real credential. **No teacher account had ever existed**, so the path had never once been executed end to end.

The verification that meant something: start an isolated server (`ADMIN_PASSWORD_HASH=$(node -e "...sha256...") DATA_DIR=/some/tmp node server/server.js` — note it falls back to port 3002 if 3001 is taken, and `.env` is never read, see the entry above), create a real teacher via `POST /api/admin/accounts`, log in for a session token, create their classroom via `POST /api/instances`, then attack it: a stranger with no credential (→ 401), a payload carrying legitimate wording **and** a re-routed destination (→ wording applied, route reverted), a structure-only payload (→ nothing minted at all), a teacher editing a slot playing an `official` card (→ branched, official's text verbatim intact), and a second save (→ updates one card, no duplicate). Reading the resulting `config.json` for each card's `owner.tier` is the receipt.

**Rule: for any change to who-may-write, the acceptance criterion is an adversarial run against a live server, not a green suite.** Two gotchas that cost time: the board CSVs are served per classroom at `/data/i/<id>/...`, and `DiceRoll Info.csv` needs its space URL-encoded (`DiceRoll%20Info.csv`) or curl silently writes an empty file, which the route then rejects with a misleading "spacesCSV and diceRollCSV are required".

### Read what a delegated agent's test EXCLUDES, not just whether it passes, before writing a release note from its summary (2026-08-26)

v3.2.40 shipped with the note "every field in the space editor says its own name." It wasn't true. The agent that did the work covered `SpaceEditor.tsx` and stopped at its edge — the outcomes section mounts `InlineDiceRollEditor` from another file — and its "nothing rendered is left nameless" test **filtered that subtree out**: `.filter(el => !el.closest('[data-editor-region="outcomes"]'))`, with an honest comment saying so. The agent was straight about the scope in the test file. The release note was written from the summary, generalized past it, and shipped a claim the test explicitly declined to make. Only found when a later pass removed the exclusion.

**Rule: when a test asserts "nothing is left X", read its selector and its filters before repeating the claim.** An exclusion line is the difference between "all of it" and "the part we looked at", and it lives in the test body, never in the summary. Applies to your own tests too — but a delegated agent's are where the gap is invisible, because you did not write the filter.

### Writing to the log changes what the dice do — one seeded `Math.random` feeds both (2026-09-03)

A one-line guard that skipped a no-op `commitCurrentSession()` call flipped `tests/ghost/ghostPlayerSmartBot` from a signature stable across 23 runs since 2026-08-26 (47 wins / 3 failures / **0 hard** / 86.9 turns) to 48 / 2 / **1 hard** / 94.9 — twice, identically — and reverting it restored the old numbers exactly. Four runs, no ambiguity. Nothing in the game had changed behaviour: nothing in the ghost bot or any decision path reads `globalActionLog`, which is write-only for decisions.

The cause is RNG-stream alignment. `ghostPlayer.ts:619` replaces the **global** `Math.random` with `mulberry32(seed)` for seeded batches — its own comment says *"Game-internal services and the ghost's own choices both observe the seeded stream."* `DiceService.ts:67` rolls from that same global. And `StateService.ts:1836` `generateActionId()` burns **one draw of it per log entry**. So the number of lines written to the log decides which numbers the dice get. Dropping one `warn()` per Try Again shifts every subsequent draw in that game and re-deals all 50.

**Three consequences, in order of how much they will cost you:**
1. **Any "tidy up the log noise" change silently re-deals every seeded game.** This inverts the usual assumption that logging is inert, so nobody connects the two. Fix the coupling before touching log volume.
...
**The reliable fingerprint is the test count, not the author line.** `npm test` (dev config, ghost excluded) reports ~201 files / ~3085 tests; a run that *includes* `tests/ghost/**` reports ~211 / ~3118. A commit message citing numbers you did not produce was written by someone else.

**Rule: lead every commit/push/status report with the absolute folder path**, and never assume a commit in the log is yours — diff the committed content against your own copy first. When two sessions share a repo, ask the maintainer to name one owner for its git rather than both pushing.


### An authored value in a CSV column the RENDERER never reads is inert — and looks exactly like a value that was never authored (2026-09-04)

The v3.2.51 button-label pass wrote all 81 approved labels into the two SOURCE files, regenerated, and got **35 labelled buttons instead of 81**. Nothing errored. The 46 dice labels were parsed, stored, regenerated into `SPACE_EFFECTS.csv`, and completely inert.

`processGameData` routed the authored dice label to **one** column:

```js
description: entry.button_label || `Roll for ${entry.die_roll}`,
```

But `formatManualEffectButton`'s dice branch **deliberately ignores `description`** — that column's auto-generated fallback is game-language ("Roll for W Cards") which used to leak onto the button, and not reading it is the fix that stopped that. The branch reads `button_label`, which the generator never emitted for dice rows. The card branch had solved this years earlier and emits both; the dice branch had only ever needed one.

**The general shape: a CSV column existing, being populated, and surviving regeneration proves nothing about whether the renderer reads it.** This repo has three prior instances of the same family — `revokes_approval` typed and consumed but never assigned by `parseCardsCsv`; `npc_speaker`/`approval_role` shipped with columns and code and **zero populated rows** for a whole version line; `fee_type`/`fee_category` hand-typed into CLEAN and erased by every regen. Each was silent for weeks.

**Check to run when authoring data into an existing column, before writing 81 of anything:** author ONE value, regenerate, and grep the generated file for the column the *renderer* names — not the column you wrote to. If the value is not in that specific column, the plumbing is missing and the other 80 will be inert too. The one-row probe costs a minute; discovering it after a full authoring pass costs a re-run of the whole pipeline.

**Second trap, same session, when verifying the fix:** seeding a deliberate failure to prove the new guard test bites did NOT trip it — because `description` and `button_label` now hold **identical text**, so a `replace(..., count=1)` hit `description` and left the real column untouched. The guard was fine; the seeding was wrong. When two columns legitimately carry the same string, a seeded-failure check must target the column **by index**, never by first-occurrence string replace, or you will conclude a working guard is broken.

### A data guard cannot see a string the RENDERER substitutes after reading the data (2026-09-06)

Direct sequel to the entry above, one stage further downstream. v3.2.51 authored 46 dice `button_label`s and shipped a guard (`tests/utils/buttonLabels.test.ts`) that reads the **real** generated `SPACE_EFFECTS.csv` — exactly the right guard, and it passed. **16 of those 46 labels still never reached a player.**

`collapsePairedDiceActions` merges two dice rows sharing one physical roll into one button (v2.70.1, correct), and on merging **overwrites the surviving row's label** with a module constant:

```ts
collapsed[i] = { ...collapsed[i], label: COLLAPSED_DICE_LABEL };  // 'Determine Outcome'
```

That fires on 8 space/visit combinations. Every CSV row passed the guard; the button a player actually read said the bureaucrat wording the guard exists to forbid.

**The general shape: "the data file is correct" and "the screen is correct" are different claims, and a data-file guard can only ever prove the first.** The 2026-09-04 entry covers a *generator* writing the wrong column. This is the mirror: the generator and the data are both right, and a *render-time substitution* discards the value afterwards. Both are invisible, both pass their tests.

**Check to run:** after authoring data, grep the SOURCE for every place the authored value can be *reassigned* — `label:`, `label =`, `?? '...'`, `|| '...'`, any constant of the same shape — not just the place it is read. A hardcoded string literal in the same domain as your authored column is a substitution site until proven otherwise. Then guard the constant itself: the new test asserts `COLLAPSED_DICE_LABEL` against the same jargon list as the CSV rows, because nothing else structurally can.

**And when the finding comes from a robot playtest, confirm the element before confirming the complaint.** The same report claimed 28 times that checkmarked controls "look completed rather than available." 25 of those were `doneActionTraces` — a `<div>`, `cursor: default`, no handler — i.e. genuinely completed, and the complaint was describing a true state. Acting on the count would have produced a change that made the UI *worse*. Reading the code first is what turned up the label-overwrite instead, which was the real bug the same string was pointing at.

### The playtest robot finds controls by reading COPY, so improving the copy blinds it — silently, and it looks like a game bug (2026-09-08)

The instrument that reports on the game is coupled to the exact thing the current workstream exists to change. `~/.hermes/scripts/game_playtest.py` (Mac mini, not this repo) locates every control by regex over **visible text**. Three consecutive releases each broke a different handle, and **all three were good changes**:

| Release | The improvement | What went dark |
|---|---|---|
| v3.2.52 | `Move — 2 options` → `…3 places to pick from` (the old wording never said *which*, an 8-hit complaint) | `MOVE_EXPANDER_RE` — the destination list never unfolded, so **the robot could not move at all** |
| v3.2.53 | gate reason `Finish N things above first` → `Pick where you're going first` | `GATED_RE` — a `disabled` button began reaching the model as a live option; Playwright waits 8s for it to enable, throws, scores a strike, three strikes end the game |
| v3.2.54 | wrapped each action row in a `<div>` to hang "What's this?" beside it | a direct-child selector; with no `.uc-hint-glow` on a revisit either, the action list came back empty |

**The symptom presents as a game dead end.** Twelve, then eighteen playthroughs "gave up" at `PM-DECISION-CHECK`, which read exactly like a broken screen — and v3.2.53 was shipped against that reading. The screen was fine for a human the whole time. **Before treating a robot's stuck-ness as a game defect, establish that the robot can still SEE the control it is failing to press.**

**Diagnostic that worked, and the one that would not have:** run the harness's own regexes against the real button strings (`ssh hermes` + a three-line python), rather than reasoning about either. Reading the report harder cannot find this — the report faithfully quoted the control in its *confusion* list while never offering it in the *click* list, and nothing marks that as contradictory. Two supporting reads: `clicked:  ➡️` counts per night (7, then 0/0/0 across the release boundary — mechanism, not correlation, which is what let this finally be attributed after two sessions had recorded it unattributable), and `drive_fails`, which is appended at :726 and **only ever `len()`'d** — the reason each click failed is collected and thrown away.

**Fix: the handles must not be copy.** v3.2.55 added `[data-testid="move-expander"]` (+`aria-expanded`), `[data-testid="move-option"]` (+`data-space-id`), `[data-testid="commit-end-turn"]` (+`data-ready`), `[data-testid="commit-side"]` (+`data-side`, `data-actionable`), `[data-testid="action-button"]` (+`data-effect-key`) — 110 insertions, 0 deletions, comments and attributes only, so it cannot change what a player sees. **A hook is only half a fix**: the harness must be changed to use it, in another repo on another machine, and neither half is observable alone.

**Two traps in that file, both older than this week.** Pickability is `disabled`/`aria-disabled` and *never* the arrow glyph, which is identical when a destination is locked and when it is merely unpicked. And `COMMIT_RE` was never sound — it matches against `end_turn_label` from `SPACE_CONTENT.csv`, i.e. per-space authored copy, so `Move forward` matched by luck while `Lock the scope` and `Take the check` never did; those spaces only worked because they render the two-tab control instead.

**Corollary for cross-session work: never cite `file:NNN` for a file another session is editing.** Twice in one day a claim about `game_playtest.py` was wrong because of *when* it was read, not what it said — including a "correction" to a correct finding, made from a file that had been half-fixed twenty minutes earlier. Quote the matched text and check `mtime`; a `.bak-*` snapshot beside the file is usually the pre-edit state and settles it.

---

**Last Updated:** September 9, 2026 (Session 2026-09-09 — **no version shipped, no game source touched.** Investigation only: settled the nightly robot's "click would not land" as an **overlay intercepting pointer events, not a game bug**, by reading Playwright's own actionability call log instead of inferring from which buttons failed. One new TACTICAL entry above covers it, including the logical exclusion that makes an inert button impossible as an explanation. The `RoutingExplanationModal` testid it recommends is **not built** — awaiting the maintainer.)

_Prior:_ **Last Updated:** September 6, 2026 (Session 2026-09-06 — shipped v3.2.52 off the 2026-09-05 robot playtest, whose own trust check disclosed 81 model errors (up from 24), 3 clicks that would not land and 17 findings dropped for not quoting the screen; counts treated as soft, quoted strings as solid, every item confirmed in code first, and one of four dissolved. **The root cause was v3.2.51's own:** `collapsePairedDiceActions` overwrites the authored dice label with a hardcoded `'Determine Outcome'` when two dice rows sharing one roll merge, on 8 space/visit combinations — so 16 of 46 authored labels never reached a player and the report's #2 string was wording v3.2.51 had already replaced everywhere the CSV could reach. New TACTICAL entry above covers that family (a data guard cannot see a render-time substitution). Also: the #1 confusion at 57 hits, "Cut back your help", was another v3.2.51 label — it never said *one* helper or that they are gone; maintainer chose "Let one helper go" / "Swap one helper for another", applied to `e_card_label` in SOURCE and regenerated. The 28-hit checkmark cluster was 25 false positives (`doneActionTraces` is a non-interactive `<div>`), but two real defects sat beside it: a live Move button announcing a move that had not happened, and finished actions listed under the header "Things you can do". The sequencing cluster's through-line — the game stating a RULE without naming its SUBJECT — is closed; all four strings now name the thing. `PROJECT_STATUS.md` and `NEXT_SESSION.md` had claimed v3.2.51 was "pending deploy" for a full day after it shipped and had already misled two sessions; corrected, with `/start` taught to verify against `/health`. Full suite `npx vitest run` 3130/3130 across 212 files, deploy verified live in both the data file and the code chunk. Prior: **Last Updated:** September 4, 2026 (Session 2026-09-04 — shipped v3.2.51, the button-label half of Onboarding Phase C. v3.2.50 had made the 27 tiles plain English; the buttons under them still said "Determine Fee Amount" on a tile called "Cut a Corner". Underneath the wording was the tiles' own defect — **0 of 80 manual rows had ever carried an authored `button_label`** — and, as with the tiles, **no schema change was needed**: all four card-label columns and the dice `button_label` already existed at 0% populated. 81 labels authored to the maintainer's rule ("buttons say what you're doing, in everyday words. No trade jargon on buttons"), which took "expeditor" off every button including `return_e`, since v3.2.47's "Let one expeditor go" fixed the direction problem but left 8 hits of "I don't know what an expeditor is". The deciding reason is structural: a glossary link can never sit inside a button, so a button is the one surface where a hard word cannot explain itself. **A real bug surfaced mid-apply** — the first regen produced 35 labelled buttons, not 81, because the generator routed the authored dice label to `description` only and the dice branch deliberately ignores that column; all 46 dice labels were written, stored and inert. One new TACTICAL entry above covers that family. Full suite 3096/3096, ghost 33/33, smart-bot byte-identical to baseline on a fourth distinct tree. Prior: **Last Updated:** September 3, 2026 (Session 2026-09-02/03 — shipped v3.2.45 and deployed it with v3.2.44. The night's real find came from a 9B local model's playtest: the commit spine replaced its caption with the gate reason, so "Lock the scope" had no name on screen until the player had already done the right thing, leaving "Push back" — which costs a day and returns you to the same space — as the only *named* control. The bot spent 11 of 17 moves in that trap; the server-side transcript of the same night showed all six games dying in the same corner. Fixed by keeping the label and moving the reason to a sub-line. Three further findings from that report were withdrawn after the maintainer challenged them and was right on all three. v3.2.46 (a Try Again log-grumble fix) is **held on `hold/v3.2.46-session-log`**, not because it is wrong but because it exposed something bigger: writing to the log changes what the dice do — one seeded `Math.random` feeds the ghost bot, DiceService, deck shuffles *and* `generateActionId()`, one draw per log entry. Two new TACTICAL entries above cover that and the two-sessions-one-repo attribution trap. Prior 3.67: **Last Updated:** August 23, 2026 (Session 2026-08-22/23 continued — shipped v3.2.35–v3.2.37 off three rounds of the maintainer's own use of the merged space-editing screen. v3.2.35: the editor and the player panel had no shared vocabulary — the editor headed its sections after the data model ("(C) Actions", "Dice Outcome Modals") while the panel named the same parts plainly — so headings now come off `spaceRegions.ts`, the one map both directions already read, and empty sections fold. v3.2.36: cleared the four `extract-zip` advisories the deploy log printed, by upgrading puppeteer FORWARD to 25.8.0 (`@puppeteer/browsers` 3.2.1 dropped the package entirely) rather than taking `npm audit fix`'s suggested six-major downgrade to 19.8.0. v3.2.37: clicking into a field now marks the part of the panel it feeds, steadily, and scrolls it into view — previously only typing lit anything; folded sections stopped being bordered cards, which was making twelve one-line rows read as clutter. Also found `express`, `cors` and `ws` filed as devDependencies while build-only frontend libs sat in `dependencies` — nothing broken by it, but it means the obvious `npm prune --omit=dev` fix for the Dockerfile's dev-dependency bloat would have deleted Express from the image and taken the server down; moved them, left the prune for a session that can build a container. Two new TACTICAL entries above, both from self-inflicted tooling wounds. Full suite 2963/2963.))

_Prior:_ **Last Updated:** August 19, 2026 (Session 2026-08-19 — shipped v3.2.14 + v3.2.15. Replaced v3.2.12's full-screen "Resume your last game?" prompt and the hidden-behind-the-gear Join-by-Code panel with a single always-visible "New game"/"Join" segmented selector (v3.2.14) — went through several live-feedback-driven redesigns in one sitting (button → merged-into-panel → selector-with-tabs) before landing; also auto-adds the first player, fixes a spectator on a not-yet-started game landing on the full editable host setup screen instead of a read-only waiting screen, and unifies the four header chips (version/Game Code/Share/gear) to one visual treatment. Then a large batch of live-playtesting-driven fixes (v3.2.15): Board Layout Editor connector naming (discipline-prefixed, since phase alone doesn't disambiguate Architect vs Engineer), alphabetized restore-picker, self-loop connectors no longer drawn, hidden edges excluded from the hover-picker, phase-colored lines, shadow-matching spacing-guide ghost, a real bug found investigating a "missing connector" report (compound `"A or B or C"` dice outcomes weren't split for board display); End Turn first-visit glow (only when Negotiate isn't offered), expeditor Activate-button glow, player-panel border/width-cap removed (confirmed edge-to-edge on phone); and the End Turn cost preview now shows the real result instead of going blank once an action completes, closing two real bugs in the process (see the two new TACTICAL entries above — the automated-browser React-Flow-edges-don't-render confirmation, and the action-log isCommitted/globalTurnNumber timing traps). Full suite 2798/2798 throughout.)

_Prior:_ **Last Updated:** August 16, 2026, continued (Session 2026-08-16 part 2 — no app version shipped, pure infra follow-through + incident response. Finished the Docker-side half of the IPv6 false-foreign-alert fix left open at the prior handoff: `game-net` recreated with `--ipv6 --subnet`, confirmed live via `docker logs game_alpha` showing a real auto-detected home IPv6 address — the whole multi-session false-foreign-alert fix is now closed end to end (TODO.md item closed out). Along the way, an attempt to also set IPv6 via `/etc/docker/daemon.json` collided with flags Unraid's own `rc.docker` already passes to dockerd, crashing the Docker daemon and taking every container on Tower down briefly — recovered via a pre-made config backup (new TACTICAL entry above has the full trap + the daemon-config-free approach that ended up working instead). Recovery surfaced a second, fully unrelated, pre-existing bug: `cloudflare-tunnel` had been crash-looping on a literal unfilled `YOUR_TOKEN_HERE` placeholder token, silently taking `game`/`dashboard`/`api`/`photos.unravelcodes.com` all offline from outside for an unknown prior period — fixed with a real token, all four confirmed reachable again (second new TACTICAL entry above). `deploy.sh`'s network-create line now always requests `--ipv6`/`--subnet` so a future accidental network removal can't silently regress the fix. No CHANGELOG entry — infra/ops only, no version bump.)

_Prior:_ **Last Updated:** August 16, 2026 (Session 2026-08-16 — shipped v3.2.8 + v3.2.9. Pulled `visitors.log` for the ~3 weeks since the admin stats dashboard shipped (v3.1.23) and used it to answer two TODO items explicitly waiting on real playtest signal: the D&D-reskin engagement question (real signal still thin — 9 real games since tracking went live — but points at join-friction, not board theme, so the reskin stays on hold) and two items with genuinely no telemetry either way (Con-Initiation crash, next-action-button highlight). Investigated Tom's "Site Stats asks for the password twice" report — root cause was `noopener` on the one `window.open()` call severing `sessionStorage` inheritance into the new tab, not a deliberate second gate; fixed (v3.2.8). A live SMS alert arriving ~16h late (T-Mobile's email-to-SMS gateway) led to timestamping the alert text (v3.2.9) and, along the way, root-causing and fixing the underlying IPv6 false-foreign-alert bug at the host level — Tower now has real, persisted IPv6 on `br0` (see the TACTICAL entry above for the full bonding/bridging/`accept_ra=2` recipe). Docker-side IPv6 (the `game-net` network itself) is NOT yet done — host networking is live, the game container doesn't have IPv6 wired in yet, next session should pick this up. `/koniec` had been skipped for this session's earlier three commits (admin double-login fix, TODO pruning, alert timestamp) — caught via `/start`'s own gap-check design, run properly now.)

_Prior:_ **Last Updated:** August 4, 2026 (Session 2026-08-04 — shipped v3.1.90 through v3.1.95. Took the G160 connector-redirect feature (code-complete but visually unconfirmed per the prior session's note) through a full real-browser debugging arc and out the other side as a permanent, server-backed board-editing system: fixed the actual "hover works, drag doesn't" bug (wrong tool was being tested — see new TACTICAL entry), then built multi-bend waypoints scaled to connector length, manual bundling/unbundling of overlapping connectors leaving the same node, box-side anchor snapping (4 fixed points per tile), an anchor-collision fix (offset-rosette spread reused from the bundling work, not a new picker menu), a restore-one-of-several picker (`RestorablePillDropdown`, shared across both admin surfaces), and closed a pre-existing gap where the standalone Board Layout Editor had no way to hide a connector at all. One v3.1.92 hotfix along the way for a live crash caused by an un-migrated data-shape change (fixed via self-healing schema upgrade in `loadInstance()`, not a manual data fix). Full suite green throughout (175 files / 2646 tests). One piece still needs the user's live confirmation: the v3.1.95 hover-highlight/distinct-cursor disambiguation.)
...
**Charter Version:** 3.95 (2026-09-25, v3.2.76 session: added one TACTICAL entry with five findings — a number authored in the words can be thrown away by the data step while the engine already has half a vocabulary for it; "the loan on the table" and the arrival-vs-leaving timing of fee and time; how to measure what real games reach by replaying the smart-bot batch with a spy on a frozen data copy; an exact-days test must call the leaving step, not a whole End Turn; and browser-verifying an arrival effect needs a real arrival, not a teleport.)

_Prior:_ **Charter Version:** 3.94 (2026-09-24, v3.2.74–75 session: added one TACTICAL entry with four findings — a push-back price is its own column (`try_again_days`), not a `time/add` row that End Turn also charges; the safe recipe for adding a per-space data column; a label the nightly robot reads has a Jarvis-side regex to check; `sed -i` flips CRLF files like the Edit tool does.)

_Prior:_ **Charter Version:** 3.93 (2026-09-22, v3.2.72–73 session: added one TACTICAL entry — a `uiStrings.ts` default change can silently do nothing because `UI_STRINGS.csv` already has a live override row for that key; check the CSV before trusting a code-only rename.)

_Prior:_ **Charter Version:** 3.92 (2026-09-19, v3.2.71 planning + slice 1: added one TACTICAL entry with three findings — raw engagement counts are mostly the nightly robot (filter by UA and home IP; 37 real games of 187), an overlaid control takes no layout room so a mirrored panel needs a real-browser look, and a page that outlived a server restart lies.)

_Prior:_ **Charter Version:** 3.91 (2026-09-19, v3.2.69–70 session: added one TACTICAL entry with four findings — measure the real count before wiring "the one special case" (8 merged dice buttons, not 1), the global test setup resets module singletons after every test, an edge-triggered open needs its previous value reset on context change, and a `&` inside `run_in_background` makes the completion notice lie.)

_Prior:_ **Charter Version:** 3.90 (2026-09-18, v3.2.66–68 session: added one TACTICAL entry with five findings — a "crash" with no error can be a legitimate game-over screen (reproduce with a headless-built state and a control), the built-in Browser pane fires zero animation frames (use the Playwright MCP), a pipeline's default branch can turn unrecognised text into the opposite action (found only by running the real row), the Edit tool rewrites CRLF files to LF wholesale, and TypeScript 6 is a real migration while 7 is unavailable to this lint toolchain.)

_Prior:_ **Charter Version:** 3.89 (2026-09-17, v3.2.61 session: added two TACTICAL entries — html2canvas clips fixed elements to an overflow:hidden ancestor, and Vite's watcher can miss an edit, so curl the served module before debugging a live check.)

_Prior:_ **Charter Version:** 3.88 (2026-09-10, v3.2.56–57 session: added one TACTICAL entry — a drifted second copy of a rule is fixed by deleting it, never re-syncing; enumerate what reaches a default (including the live classroom copy) before flipping it; a CSV column can be a gate and a parameter at once; prove a regression test by running it against the old code; and relayed briefs are hypotheses — four confident claims in this session's brief were wrong. Also rewrote the NPC-speaker note: there is now one speaker map, CHARACTERS.csv.)

_Prior:_ **Charter Version:** 3.87 (2026-09-08, v3.2.55 session: added one TACTICAL entry — the playtest robot finds controls by matching visible TEXT, so three consecutive good copy/DOM improvements (v3.2.52/53/54) each silently blinded a different handle, and the symptom presented as a game dead end that v3.2.53 was shipped against. Covers the diagnostic that works (run the harness's own regexes against the real strings, not reasoning about either), the destination-click counts that finally made the step-count collapse attributable by mechanism rather than correlation, the five `data-testid` hooks that decouple the two, the two older traps in that file (the arrow glyph cannot express lockedness; `COMMIT_RE` matches per-space authored copy and never worked on most spaces), and the cross-session corollary that `file:NNN` citations are worthless while another session edits the file.)

_Prior:_ **Charter Version:** 3.86 (2026-09-07, v3.2.53–v3.2.54 session: added four TACTICAL entries — a domain rule hand-written in N files gets missed in the N+1th, and the miss lands in the presentation layer (the skippable-action rule, which put an impossible instruction on the commit spine of all 12 choice-movement spaces); a branch ordered after a broader condition can be permanently unreachable, so grepping for a user-facing string proves it exists and never that it renders; `TextWithTerms` cannot live inside a `<button>` because its term span calls `stopPropagation()`, so an explanation goes beside a control rather than on it, and its `onTermClick` is optional so omitting it yields underlined terms that do nothing; and the glossary matched no plurals for the life of the feature, fixed by widening the word INDEX rather than loosening the regex, because an index is measurable. Also strengthened the 2026-08-26 `io.open(path,'w')`-truncates entry after it recurred TWICE the same day — now root-caused to `\uXXXX` escapes in a Bash-tool heredoc arriving as real lone surrogates.)

_Prior:_ **Charter Version:** 3.85 (2026-09-04, v3.2.51 session: added a TACTICAL entry on the family where an authored CSV value is inert because the RENDERER reads a different column than the generator writes — 46 dice labels were parsed, stored, regenerated and did nothing, with no error, which looks identical to never having authored them; includes the one-row probe that would have caught it in a minute, and the seeded-failure trap where two columns holding identical text made a working guard test look broken. Also corrected the voice-rule override table: authored `button_label` now wins over `buttonFormatting.ts`, whose wording is only a fallback for unauthored rows.)

_Prior:_ **Charter Version:** 3.84 (2026-09-01, v3.2.44 session: corrected the 2026-07-15 "4K TV self-reports a small viewport" entry — "renders at reduced resolution" was the wrong model and cost two sessions; `screen.width x devicePixelRatio` says the panel is 3840 wide and nothing is blurry, so what is small is the LAYOUT BUDGET, which makes it a one-line viewport-meta fix rather than a hardware limit — plus the corollaries that widening the viewport trades legibility proportionally (true 4K = 0.13 deg, unreadable) and that physical size/viewing distance is unmeasurable, so ask the viewer instead of building a device-model lookup table. Added a second entry: a summary endpoint that hand-lists a subset of a record loses fields SILENTLY — `/api/public/feedback/open` never returned `extra`, so reports were closed on half their content for months, and 42 of 222 turned out to carry unread text.)

_Prior:_ **Charter Version:** 3.83 (2026-08-26, v3.2.38–v3.2.42 session: added four TACTICAL entries — `io.open(path,'w')` truncates before the write, so a patch script that raises mid-write leaves the file EMPTY (this one destroyed a 3435-line committed doc, recovered via git); "the suite is green" depends on WHICH suite, since the batch runner enumerates files explicitly and misses whole directories; a permission boundary cannot be proven by unit tests — stand up a real server with a real account and try to break it; and read what a delegated agent's test EXCLUDES before repeating its claim in a release note, which is how v3.2.40 shipped an overstated one.)

_Prior:_ **Charter Version:** 3.82 (2026-08-23, v3.2.35–v3.2.37 session: added two TACTICAL entries — the Browser pane never holds keyboard focus, so `.focus()` moves `activeElement` while firing zero focus events (root-causing the long-standing "synthetic keystrokes don't reach the page" obstacle, with the `dispatchEvent(new FocusEvent('focusin'))` workaround), and the patch-script pair: preserve each file's own CRLF/LF or bury a 500-line change in a 2500-line diff, and never inline a backtick-bearing script into `python -c` from bash because bash executes the backticks first.)

_Prior:_ **Charter Version:** 3.81 (2026-08-16 continued, Docker-side IPv6 + tunnel-outage session: added a 4th trap to the IPv6 TACTICAL entry — Unraid's `rc.docker` already passes `--ipv6`/`--fixed-cidr-v6` flags to dockerd, so mirroring them in `daemon.json` crashes the daemon instead of merging — plus a new entry on checking `docker inspect`'s `Cmd` before trusting a "token invalid" error at face value.)

_Prior:_ **Charter Version:** 3.80 (2026-08-16, host-IPv6 + SMS-alert session: added a TACTICAL entry on enabling real IPv6 on a bonded/bridged Docker host — the `eth0→bond0→br0` chain, the `forwarding`+`accept_ra=2` gotcha, and a GUI toggle silently disagreeing with the actual running service.)

_Prior:_ **Charter Version:** 3.79 (2026-08-14, Workstream 6 reskin-audit closeout + housekeeping session: added the versioning-cap rule right after the existing "bump package.json when releasing" rule, plus two TACTICAL entries — the shared CSV parser's unescapable literal-`"` limitation, and testing a React component against mutable module-level config state without tripping a duplicate-React-instance failure.)

_Prior:_ **Charter Version:** 3.78 (2026-08-04, no-ship investigation session: added "Working Style: Research, Then Push Back" under Mission & Responsibilities, plus two TACTICAL entries — the PowerShell `curl`-alias gotcha and reading `visitors.log` over SSH to sidestep admin-password-gated stats endpoints.)
