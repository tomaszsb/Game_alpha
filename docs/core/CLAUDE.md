...
**Charter Version:** 3.81 (2026-08-16 continued, Docker-side IPv6 + tunnel-outage session: added a 4th trap to the IPv6 TACTICAL entry — Unraid's `rc.docker` already passes `--ipv6`/`--fixed-cidr-v6` flags to dockerd, so mirroring them in `daemon.json` crashes the daemon instead of merging — plus a new entry on checking `docker inspect`'s `Cmd` before trusting a "token invalid" error at face value.)
