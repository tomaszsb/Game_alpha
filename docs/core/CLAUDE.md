...
**Last Updated:** July 2, 2026 (Session 2026-07-02 — **shipped v3.0.92, pending deploy.** Loss screen for no-winner endings (`gameEndReason` → EndGameModal loss variant; bankruptcy used to end on a blank page). Funding-gap "deficit" cue driven by `computeProjectFinances`. End-turn button shows "this turn: 🕐+Xd · 💰−$Y" (`getTurnOutflow` + REAL-snapshot diff + the space's unconditional auto time rows). Contractor redesign per maintainer "A and C": shared `contractorTerms.ts`, price 72–150% of estimate, schedule 8–78 days, signing = mandatory bill → bankruptcy. Decisions: loans don't bankrupt (repayment post-scope), low cash OK. Targeted sweep 1772/1772; strict ghost floor passed mid-run.)
...
**Charter Version:** 3.51 (updated the negative-money-guards note for v3.0.92 — loan/low-cash decisions recorded, contractor signing joined the mandatory bills, `contractorTerms.ts` is the shared price/schedule math. Prior 3.50: TWO negative-money guards, the v3.0.91 economy change. 3.49: "no dice numbers or board-game icons in player copy" hard rule.)
