...
**Last Updated:** June 20, 2026 (Session 2026-06-20 — **no new version shipped (still v3.0.80).** Built all of **Phase 4b** on branch `phase-4a-card-insertion`: authored spaces can now splice onto dice/choice edges, deal cards, be a dice roll (per-face), and charge fees (flat / % of loans / % of scope). 6 feature commits + a typecheck fix; full ghost gate green each slice. Not merged/pushed/deployed. Also rewrote README into a self-maintaining pointer + added a monthly-maintenance step to `/start`. **Note:** the `.claude/plans/replicated-baking-hoare.md` referenced by the prior handoff never existed — the plan was reconstructed from the Phase 4a code.)
**Charter Version:** 3.45 (added the "Authored-space bakes" TACTICAL note: curated DICE_OUTCOMES is load-bearing for dice spaces; percentage fees map by Fee string not fee_calculation_method; re-run typecheck after tests touching JSDoc-typed server params. Prior 3.44: Express-vs-Vite board serving + UPPERCASE authored ids + allowlist-clean authored rows + Windows EPERM bake note.)
