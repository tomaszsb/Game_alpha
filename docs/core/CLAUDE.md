...
**Charter Version:** 3.69 (added the "`npm audit fix --force` can silently install a broken peer-dependency combo" TACTICAL entry — the ESLint 9→10 attempt that crashed lint via `eslint-plugin-react`'s missing ESLint-10 support, verified against the npm registry, 2026-07-25 session, v3.1.40.
