...
**Last Updated:** June 7, 2026 (Session **v3.0.69** — dashboard housekeeping [8-report flip]; ghost `try-again-happy` gate investigated + split into blind `negotiate-coverage` [0-hard-failures only] + `smart-bot` win-rate [smartTryAgain skips undo-after-approval]; CardEffectHandler receipt off-by-one fixed [snapshot moved pre-draw]; fb:931a55de [6 more L-cards de-jargoned + L049 bare `draw_cards=1`→`1 E` so it draws an Expeditor not a Work Package + by-ID integrity pin].)
**Charter Version:** 3.33 (added: ghost win-rate drop is a bot-policy question before a balance one — blind Try Again at examiner spaces reverts just-earned approval [TEMP] → Stage-1 gate loop; split coverage [0-hard-failures] from win-rate [smart bot]; hoisting a call out of a short-circuited `&&` runs it for other branches. Prior 3.32: board tile label = display_label_override not SPACE_CONTENT.Title; board edges read MOVEMENT.csv only so dice spaces need DICE_OUTCOMES fallback; tiles grow with content so buffer ghost must be content-aware; ApprovalStatus is lowercase 'approved' — uppercase comparison = dead code; editor saves combined Spaces.csv → server regenerates split CLEAN_FILES → reloadAllData.)
