...
**Last Updated:** June 18, 2026 (Session 2026-06-18 — **no new version shipped (still v3.0.80).** Verified **Phase 4a** in the running game before starting 4b (user call). Found + fixed **four real bugs** the build-only tests missed — lowercase-id soft-lock, resolver clone-and-blank behavioral leak, position overlap, label-confirm — on branch `phase-4a-card-insertion` (commit `af3e5f6`; 227 server + 1658 unit green, typecheck + build clean). Did NOT start 4b. 4b is now **sliced** (fork-splice → card-draws → dice-outcomes → percentage-fees); plan in `.claude/plans/replicated-baking-hoare.md`.)
**Charter Version:** 3.44 (added: "Verify per-instance boards via Express (3001) not Vite (3000) — Vite serves /data from public/" + "authored ids must be UPPERCASE / pipeline parsers are uppercase-only" + "resolver builds authored rows allowlist-clean, never clone-and-blank-denylist" + the Windows resolved-dir EPERM bake note. Prior 3.43: capturing transient modals via gameState injection.)
