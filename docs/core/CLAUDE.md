...
**Charter Version:** 3.80 (2026-08-16, host-IPv6 + SMS-alert session: added a TACTICAL entry on enabling real IPv6 on a bonded/bridged Docker host — the `eth0→bond0→br0` chain, the `forwarding`+`accept_ra=2` gotcha, and a GUI toggle silently disagreeing with the actual running service.)
