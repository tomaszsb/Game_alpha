...
**Charter Version:** 3.84 (2026-09-01, v3.2.44 session: corrected the 2026-07-15 "4K TV self-reports a small viewport" entry — "renders at reduced resolution" was the wrong model and cost two sessions; `screen.width x devicePixelRatio` says the panel is 3840 wide and nothing is blurry, so what is small is the LAYOUT BUDGET, which makes it a one-line viewport-meta fix rather than a hardware limit — plus the corollaries that widening the viewport trades legibility proportionally (true 4K = 0.13 deg, unreadable) and that physical size/viewing distance is unmeasurable, so ask the viewer instead of building a device-model lookup table. Added a second entry: a summary endpoint that hand-lists a subset of a record loses fields SILENTLY — `/api/public/feedback/open` never returned `extra`, so reports were closed on half their content for months, and 42 of 222 turned out to carry unread text.)
