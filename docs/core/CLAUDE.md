...
**Last Updated:** June 13, 2026 (Session 2026-06-13 — deploy/ops: **v3.0.74–76 now LIVE**. Deploy surfaced two infra bugs: deploy.sh wipes `instances/` + resurrects old SOURCE/CLEAN, and the migration reads positions from SOURCE while the editor saves to CLEAN → the maintainer's custom board layout was lost ~June 12, unrecoverable. Wrong-container trap documented (never `docker compose up`; NPM→3080; cf-cache DYNAMIC). Both bugs tracked in TODO.)
**Charter Version:** 3.39 (added: deploy is NOT teacher-layer-aware — deploy.sh must preserve instances/ + drop the old SOURCE/CLEAN restore; migration reads SOURCE not CLEAN; wrong-container/NPM-routing/cf-DYNAMIC deploy-diagnosis playbook; teacher layer marked LIVE, data-deploy-gap death still unverified. Prior 3.38: teacher instance layer data model.)
