...
**Charter Version:** 3.93 (2026-09-22, v3.2.72–73 session: added one TACTICAL entry — a `uiStrings.ts` default change can silently do nothing because `UI_STRINGS.csv` already has a live override row for that key; check the CSV before trusting a code-only rename.)
