...
**Charter Version:** 3.75 (2026-08-01, v3.1.86 session: added a TACTICAL entry confirming local `server/data/` is unrelated to the production copy — `deploy.sh` mounts a separate volume on the Unraid host, so local dev-server logs are safe to clean up without touching anything live.)
