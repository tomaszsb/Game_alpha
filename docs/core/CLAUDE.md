...
**Drift found doing it (tracked in TODO, not yet fixed):** `validateInsertions` ([instanceValidation.js](../../server/instanceValidation.js):212) decides "is `from` a dice source?" from `requires_dice_roll === 'YES'` and then looks for the A→B edge in the *dice table*. But a space can have `requires_dice_roll=Yes` for a **non-movement** roll (`die_roll='W Cards'/'Time outcomes'/'Fees Paid'` — e.g. `OWNER-SCOPE-INITIATION`) while its real onward edge is the **fixed** `space_1`. The engine/resolver key dice-*movement* on `die_roll==='Next Step'`, so the two disagree and splicing onto that fixed edge is rejected with a misleading `INSERT_EDGE_MISSING` ("no … dice outcome to splice into"). Workaround: splice onto a truly-fixed edge (e.g. `FUND→PM`). Fix: make validation's `isDice` read `die_roll==='Next Step'` from the dice CSV, else fall back to the `space_N` edge check.

---

**Last Updated:** June 20, 2026 (Session 2026-06-20b — **no new version shipped (still v3.0.80, Phase 4 live at `9666140`).** Verification session: walked a Phase 4b authored space in a local running app (Express localhost:3001) — splice routing, auto card-deal, `SCOPE_PERCENTAGE` fee, and 6-face dice routing all confirmed end-to-end; only the live-prod walk remains. Found a validator/engine dice-detection drift (see the new TACTICAL recipe note above), tracked in TODO. No source changes.)
**Charter Version:** 3.46 (added the "Walking an authored space locally (recipe) + validator/engine dice-detection drift" TACTICAL note. Prior 3.45: "Authored-space bakes" — curated DICE_OUTCOMES load-bearing; percentage fees map by Fee string; re-run typecheck after JSDoc-typed-param tests. 3.44: Express-vs-Vite board serving + UPPERCASE authored ids + allowlist-clean authored rows + Windows EPERM bake note.)
