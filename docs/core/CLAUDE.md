...
**Last Updated:** June 12, 2026 (Session 2026-06-12 — **v3.0.71**: both 2026-06-11 playtest bugs shipped + deployed + live-data synced + dashboard reports resolved [open 7→5]. Bank-loan label fixed in data [SOURCE ModalConfig + CLEAN SPACE_EFFECTS → "Accept Bank Loan", live sync verified at origin] + source-aware code fallback. Result-modal flash: live repro overturned the queued AnimatePresence-swallow diagnosis — real cause is click-through onto the just-opened modal's backdrop; fixed with `BACKDROP_GRACE_MS` 500ms in ModalBase [all modals] + `useModalQueue` result queue as defense-in-depth. Windows ssh config created → `ssh unraid` resolves from PowerShell + Git Bash.)
**Charter Version:** 3.36 (added: repro the bug before building the prescribed fix — a queued diagnosis can be wrong [fb:ac29b623: "AnimatePresence swallow" theory disproven live; real cause click-through → backdrop grace]; stale WSL netsh portproxy rules hijack dev ports 3001-3004 → server falls to 3005, vite /api proxy dead; Playwright `page.route` remap workaround. Prior 3.35: CSV data fixes don't reach the live server on deploy — it serves a preserved writable CLEAN copy; sync manually [compare vs live, never blind-copy GAME_CONFIG board layout, fix SOURCE for durability].)
