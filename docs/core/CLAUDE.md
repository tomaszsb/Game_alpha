...
**Charter Version:** 3.79 (2026-08-14, Workstream 6 reskin-audit closeout + housekeeping session: added the versioning-cap rule right after the existing "bump package.json when releasing" rule, plus two TACTICAL entries — the shared CSV parser's unescapable literal-`"` limitation, and testing a React component against mutable module-level config state without tripping a duplicate-React-instance failure.)
