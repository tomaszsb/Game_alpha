...
- **Stock** (writable SOURCE/CLEAN) **refreshes on every deploy** via the server's on-boot stock-refresh (NOT via deploy.sh — see the deploy entry above; deploy.sh actually restores the OLD SOURCE/CLEAN, which the server then overwrites). ⚠️ The data-deploy gap is therefore *mostly* dead but **never actually verified end-to-end** ("does a repo CSV fix reach live?") — confirm after fixing deploy.sh. The manual live-sync recipe below is effectively obsolete but kept until that verification.
...
- **CardModal** ← `gameState.activeModal = {type:'CARD', cardId}` (cardId must exist in CARDS_EXPANDED, e.g. `E008`).
...
- **EndGameModal** ← `gameState.isGameOver = true` (+ `winner`, `gamePhase:'END'`).
...
**Still inconsistent (follow-up):** the `FEE_DEDUCTION` loan path (`applyFeeDeduction`) still pre-checks `canAfford` and *fails with a message* rather than bankrupting — a more complex mechanic (tiered/dice fees) left out of the v3.0.91 change. If loans should also bankrupt, that's the next edit. Diagnostic for the class: the `spendMoney`-returned-false error prints `payload.amount` — which is `0` for percentage-of-scope fees (real amount is computed into `actualAmount`), so a "MONEY change of 0" console error is a *real fee* failing, not a literal zero (now fixed to print `actualAmount`).

---

**Last Updated:** July 1, 2026 (Session 2026-07-01 — **shipped v3.0.91, pending deploy.** Money model: unpayable mandatory bills now bankrupt (real `endGame`) instead of being silently dropped — the "money doesn't reconcile" fix; `allowNegative` threaded through both negative-money guards. Plus the first before→after outcome modal (new-view `OutcomeChangesV2`, names the card gained/lost), a green→orange→red money runway cue, source-typed active-effect emoji, day-delta colour, "Approve"→"Accept" copy. Triaged the 63-report v3.0.90 playtest → 48 TODO candidates. Typecheck + build clean; targeted sweep 1757/1757; strict ghost gate passed.)

_Prior:_ **Last Updated:** June 29, 2026 (Session 2026-06-29 — **shipped v3.0.87, pending deploy.** Cluster B leftovers (glossary dup-key, roll-result clarity, expeditor-replace phase chip) + a full no-game-language copy sweep (no die number/🎲/🃏 in player copy; Life Event=📰, generic=📄). Process: TODO pruned of ~248 done items + Parking lot; `/koniec` now deletes completed items; ghost progress heartbeat. Typecheck + build clean; targeted sweep 1719/1719 green. Decision: player panel NOT ready to flip to default — stays opt-in.)
**Charter Version:** 3.50 (added "Money spend has TWO negative-money guards; mandatory bills bankrupt, discretionary buys block" — the v3.0.91 economy change. Prior 3.49: "no dice numbers or board-game icons in player copy" hard rule. 3.48: "card-play phase gating lives in the shared canPlayCard + two unrelated SETUPs".)
