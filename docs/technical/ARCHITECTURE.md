...
| **CARD_TYPES.csv** | Card-type display labels ("Work Package", "Bank Loan", etc.) via `DataService.getCardTypeLabel()` — added so real-life-voice card-type names live in data, not hardcoded literals. Also `is_playable_from_hand` (v3.2.56): which card families get an Activate button, via `DataService.isCardTypePlayableFromHand()` — the UI must never gate on `card_type === 'E'`. Clean-only (no SOURCE copy). |
