...
- **Test suite:** `npm test` **3084/3084 across 201 files**, 0 failures. Typecheck ✅ build ✅. ⚠️ **`npm run test:ghost` was still running when the session wrapped** — output at `/tmp/ghost47.log`; re-run if in doubt. It is expected clean (v3.2.47 changed two string literals only, consuming no extra `Math.random` draws — see item 1), and the smart-bot signature on this code measured 47/3/0 hard/86.9 earlier the same night. Note: the held branch reports **3085** — it carries the new Try-Again regression test.
