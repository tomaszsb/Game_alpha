...
## State at handoff
- **Version:** v3.2.72 — **pending deploy** (built + tested this session, not yet pushed by Tom). Run `curl -sS https://game.unravelcodes.com/health` before believing this — live is still v3.2.71 (`d4f4759`) as of this handoff.
...
- **Last shipped:** v3.2.72 — Onboarding Phase C Slice 3, partial. Closed the top 2 confusions from the 2026-09-21 playtest report: the press-and-hold control's hint is now a real instruction (was a 9.5px muted footnote) plus its own "?"; the four glance boxes (Money/Time/Expeditors/Scope) share one "?" explaining each, including what "deficit" means (and green/red states, per Tom's live feedback). Also: every "?" in the game shrank from a 44px touch-target floor to 26px, matching the header toolbar's small icon buttons — Tom's direct feedback mid-session ("too large... make them smaller like the info icon for the glossary").
- **Test suite:** `npm test` **219 files / 3317 tests green** (+3 pinning the new "?" behaviors). Typecheck ✅, build ✅. Ghost gates (`npm run test:ghost`) **not re-run** — judgment call, this change is presentation-only (help text + button sizing), no game logic/dice/movement/effects touched. Run it before the NEXT logic-touching change if you want fresh confirmation.
- **Still Tom's:** deploy v3.2.72 and confirm it feels right in his own hands. React to slice 1 **on a real phone** (light and dark) is still outstanding too, and the ~20px phone-width oddity (TODO, active bugs) — confirmed it still clips the new commit-control "?" on a 375px emulated viewport this session (pre-existing bug, not new).
...
1. **Deploy v3.2.72.** `ssh unraid "cd /mnt/user/appdata/Game_alpha && bash deploy.sh"` from a Windows terminal (yours, not this session's).
...
3. **Slice 3's remaining two gaps** (move choices, Rules → "How to play") — both have a clear starting point already identified (see above), no re-investigation needed.
