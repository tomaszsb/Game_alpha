...
- **Separate parallel session (no repo changes):** set up `tower.unravelcodes.com` for external access to Tower's Unraid admin GUI, behind Cloudflare Access. Full detail in AI memory (`project_tower_remote_access`) — includes a discovery that DNS record creation for `unravelcodes.com` is now scriptable via an existing scoped Cloudflare token (updates the older "maintainer-only" assumption in the D&D-reskin infra notes), and a DNS-01-vs-Access-gate cert gotcha worth knowing before protecting any other subdomain with Access.
