# Next session starter — written 2026-09-17 by /koniec (amended 2026-09-18 after v3.2.67)

## State at handoff
- **Version:** v3.2.67 — **PENDING DEPLOY, together with v3.2.66 and v3.2.65** (all pushed 2026-09-18). Live is v3.2.64 (`/health` = `3a87964`). **Run `curl -sS https://game.unravelcodes.com/health` before believing this line.**
...
- **Last built:** v3.2.67 — TypeScript 6.0.3 + jsdom 29 + framer-motion 13.4 (each tested; nodemailer 10 deferred), lint passes again, `NUMBERS.*` defaults synced + a drift guard, and the Con-Initiation "crash" closed as the bankruptcy ending (reproduced with a control). **Watch after deploy:** the bundle now carries React 19.3 / Vite 8.3 / framer-motion 13 — open a modal or two on a real phone. v3.2.66 — same-major dependency refresh (113 packages; React 19.3, Vite 8.3; audit still 0) and `USER_MANUAL.md` rewritten against the live panel (it had the win rule wrong). **Found, not fixed:** four "Pass help to your left/right" buttons give the presser a FREE expeditor instead of passing one on (data pipeline never emits the engine's `transfer` action) — a balance decision, written up in TODO 🙋 with options and a recommendation. **Also found:** this machine tests on Node 20.19, production runs Node 24. v3.2.65 — `npm audit` 9 → 0 vulnerabilities; fixed fb:ae480630's hidden request (commit control now highlights whenever `commit.ready`, not just on a First visit); added `byOrigin` (home/foreign) to `/api/admin/engagement-stats`, the prerequisite a 2026-08-15/09-01 TODO item was waiting on; "underwriting" (Bank Review's modal title, unreachable by the glossary linker) → "reviewing"/"reviewed", Tom's pick from this morning's Jarvis report. v3.2.64 — the three worst copy offenders from the 2026-09-12 playtest.
- **Test suite (v3.2.67):** `npx vitest run` **3215/3215 across 224 files** (three `tests/server/**` EPERM rename errors in `instanceResolver` — confirmed non-regression via isolated re-run, 160/160; the documented Windows temp-dir pattern); typecheck ✅; lint 0 errors.
...
## Top open items
1. **Teaching layer: tutorial, micro-lessons, and the "What's this?" tooltips.** A per-row DRAFT is waiting for Tom's ok/edit/no (`docs/core/AUTHORED_COPY_REVIEW.md`, last section). It is smaller than "44 rows": only 6 rows are reachable, and the "?" on the 45 dice actions repeats the button's own label (proved live). Part 1 can ship the moment he answers; Part 2 needs a small wiring change. Never put a glossary term inside an action button.
2. **Tom's call — "Pass help to your left/right" gives a FREE expeditor** (4 spaces, Subsequent visits; TODO 🙋 has the mechanism, options A/B and my recommendation A). Fixing it removes something players currently get, so ghost win-rate/turns should be run before/after and reported before shipping. Not built.
