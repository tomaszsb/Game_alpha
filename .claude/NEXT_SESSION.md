...
Deploy v3.0.81 (already pushed — just `ssh unraid …`), then do the live-prod authored-space walk to close the last Phase-4 verification — or, if you'd rather build, jump straight to the deploy-independent UI redesign. Which appeals?

## Reminders
- v3.0.81 is already on origin/master — `deploy.sh` builds from pushed code, so it's ready. Deploy runs from the **Windows terminal**: `ssh unraid "cd /mnt/user/appdata/Game_alpha && bash deploy.sh"`.
