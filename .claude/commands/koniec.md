...
| Shipped a user-visible feature/fix | `docs/user/RELEASE_NOTES.md` | **append** a release intro — plain-language, player audience, different voice from CHANGELOG. Only if a playtester needs to know. **Also glance at the top of the file and check the PREVIOUS shipped version got one** (added 2026-09-04): a skipped note leaves no trace, so it is invisible unless you look. v3.2.50 renamed all 27 board tiles — as player-facing as it gets — and its `/koniec` wrote CHANGELOG but no release note; the gap was only found a session later by opening the file for an unrelated reason. If the top entry is more than one version behind `package.json`, backfill the missing one while you are here. |
