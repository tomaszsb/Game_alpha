...
- **Verify that claim against the LIVE server — never against the note (added 2026-09-06):**
  ```bash
  curl -sS --max-time 15 https://game.unravelcodes.com/health
  ```
  Compare the returned `version` (a commit hash) to `git rev-parse --short HEAD`. **Equal ⇒ the tree is deployed, whatever the handoff says.** This step existed in spirit but never actually ran a check: it compared `package.json` to the *note's own claim*, and both are written by the same `/koniec`, so they go stale together and agree with each other while being wrong. On 2026-09-04 v3.2.51 shipped and `/koniec` wrote "PENDING DEPLOY. Live is still v3.2.50 (`b4765b5`)" into **both** `NEXT_SESSION.md` and `PROJECT_STATUS.md`. `/health` was already `e5cc6a0` = HEAD. The claim survived a full day and **two** `/start` briefings, each of which dutifully repeated "suggested first move: deploy it" for something already live. One `curl` settles it, and every downstream step here (the flip-after-deploy list, `/koniec` step 4a's "confirmed live" gate) already depends on knowing the answer. If `/health` disagrees with the handoff, say so in the briefing and **fix both files as part of the session** — a wrong deploy claim is not a cosmetic docs error, it is the input to the first action of the next three sessions.
...
- **Untriaged-feedback count — every session, one line (added 2026-09-17):** if `.env` has `FEEDBACK_TOKEN`, fetch the open list (same `curl` as step 4c) and count reports whose trailing hex appears in **neither** `TODO.md` nor `CHANGELOG.md`. Zero → silent. Non-zero → one line: `📬 N open report(s) not tracked anywhere — run /start full or triage now`. Do NOT draft bullets or cluster here; that stays in the gated step 4. **Why:** the full sweep only runs monthly, so reports filed just after a monthly pass are invisible for weeks. Ten reviewer reports from 2026-09-04/08 sat untracked until a manual check on 09-16, though three `/start` briefings ran in between. Counting costs one request.
- **Skipped-`/koniec` gap check (added 2026-07-25; made commit-exact 2026-09-04):** compare **commits, not dates**. Run `git log --oneline $(git log -1 --format=%H -- .claude/NEXT_SESSION.md)..HEAD` — any output means work landed after the handoff note was last written, so `NEXT_SESSION.md` may be stale relative to `git log`. Flag it (`⚠️ N commits since the last /koniec handoff — NEXT_SESSION may be behind git log`) and reconcile before trusting the recap in step 2. Cross-check `package.json`'s version against the version the note claims: a mismatch is the loudest single signal. **Why not dates:** this step used to compare `git log -1 --format=%ad --date=short` against the `written YYYY-MM-DD` on line 1 of the note, which is blind at day granularity. On 2026-09-04 the note was written 2026-09-03 and the most recent commit was *also* dated 2026-09-03 — so the check stayed silent while **six commits and three shipped versions (v3.2.48/49/50)** sat past the handoff, two of its three "top open items" already closed. Same-day shipping after a `/koniec` is exactly the case this check exists for, and the date form cannot see it. The commit form above is precise, costs one command, and has no granularity blind spot.
