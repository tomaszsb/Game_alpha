...
### Stale-done TODO item closed (fixloop iteration, same day)
**"Dashboard UI: surface `version` + `gitCommit` on bug-report pages"** — verified already shipped in the `dictionary-scraper` repo (commit `37b9d0f`, "surface version + gitCommit badge on feedback list & detail"); both the feedback list and detail pages already display the version badge and commit tooltip. No code change needed here — removed the stale line from TODO.md.
...
Fourth and final planned stage of [docs/design/domain-events.md](docs/design/domain-events.md). Moves *emitter ownership* so the class that decides what happened also decides whether/what to announce, instead of a caller re-deriving that logic on the callee's behalf — the last structural gap the design doc identified.

**Real bug fixed, not previously known:** `FinancialEffectHandler.checkBankruptcy`/`checkDesignFeeCap` both repurposed the stage-2 `life_event` event shape for their game-ending announcements. But `ToastWriter`'s `life_event` case reads `event.cardType`/`event.cardName` — fields these two emissions never set — and never read `event.message`, so bankruptcy and design-fee-cap losses rendered a toast literally saying **"Received: undefined"** instead of the crafted message. Never reached players (v3.1.5–8 are all still pending deploy). Fixed by giving these moments — plus the WIN ending, which previously had **zero** announcement anywhere outside `EndGameModal` — a proper `GameEnded` event (`reason: 'win' | 'bankruptcy' | 'design_fee_cap'`), with real `LogWriter`/`ToastWriter` cases. `TVDisplay.tsx`'s dramatic overlay (which correctly showed bankruptcy/design-fee-cap via `life_event` before) got `'game_ended'` added to its same condition so it doesn't regress — and now flashes on a win too, previously invisible there.

**ApprovalService ownership moved without compromising its purity.** `ApprovalService` is fully stateless (no constructor, zero dependencies) — its 4 approval-domain moments (examiner dice-roll outcomes, Prof-Cert self-certification grants, the REG-DOB-FINAL-REVIEW gate bounce, and DOB/FDNY revocations) were all announced by callers (`DiceRollProcessor`, `CardService`) re-deriving gating conditions and message text at each call site. Rather than inject a `stateService`/emit dependency into `ApprovalService` (which would make a currently side-effect-free, trivially-testable class stateful for no behavioral gain), its methods now return event-shaped data alongside their existing state patch — `revoke(player, target, context)` returns `{update, event}` (the "was there something to revoke" gating check and message text, previously duplicated at 2 call sites, now live once inside `revoke()`; `event` is `null` for a genuine no-op); `grantProfCertApproval(playerId, player)` similarly returns its own announcement text instead of `DiceRollProcessor` hardcoding it inline. Callers became thin, mechanical forwarders.

**Per the maintainer's decision, 4 previously-silent approval/game-ending moments now reach the permanent log** (same category of decision as stage 3's Movement question): examiner approval outcomes + Prof-Cert grants (new `ApprovalOutcomeDetermined` event — previously shown only in the dice-result modal banner), the final-review gate bounce (previously toast-only), approval revocations (previously a dedicated on-screen notice only), and the WIN ending. Three genuinely new log tags (`approval_revoked`, `approval_outcome_determined`, `routed_back_to_review`) needed registering in `StateTypes.ts`'s `ActionLogEntry` union, `LoggingService`'s action-tag allow-list, and `actionLogFormatting.ts`'s labels/icons — `game_ended` needed none of that: it reuses the `'game_end'` tag, which turned out to already be fully registered (union, allow-list, label, 🏆 icon) and simply never fired from anywhere until now.

**CardService's 12 card-lifecycle log calls promoted to typed events** (`CardDrawn`, `DeckReshuffled`, `CardReplaced`, `CardTransferred`, `CardExpired`, `CardDiscarded`, `CardEffectTargetResolved`) — all log-only, zero companion toasts (confirmed by grep), reusing every existing `action:` tag verbatim including the two pre-existing inconsistencies (`CardReplaced`'s "two log lines for one action" quirk, `CardEffectTargetResolved`'s divergent no-target/resolved tags across its 2 distinct mechanics) — deliberately preserved as-is this stage, not silently fixed. **`CardPlayed` + `CardActivated` merged** into one event with an `activated: boolean` fact for the `CardService.playCard()` path specifically (traced the exact call graph: a duration card always fires both today, a non-duration card only fires the first — the merge loses no information) via an additive, backward-compatible `suppressActivationEvent` option on `activateCard()`/`finalizePlayedCard()`; every other caller (e.g. `CardEffectHandler`'s standalone `CARD_ACTIVATION` effect handler) is unaffected and still gets its own standalone `card_activated`.

**Flagged, not fixed:** a static trace during the `CardPlayed`/`CardActivated` work surfaced a *separate*, pre-existing possible double-activate bug in `PlayerActionService.playCard()` (the classic-panel's independent card-play path, which never calls `CardService.playCard()` at all) — spun off as a follow-up investigation, out of scope for this stage.

Verified: typecheck clean, production build clean, full suite **2427/2429 passed** (1 pre-existing skip, 1 single-test timeout in `E2E-AllPaths.test.ts` reproduced as non-deterministic across 3 isolated reruns — matches the already-documented pre-existing scheduling flake in TODO.md, confirmed non-regression). 34 new/updated unit tests across `ApprovalService.test.ts`, `CardService.test.ts`, `FinancialEffectHandler.test.ts`, `LogWriter.test.ts` (18 new cases), and `ToastWriter.test.ts` (6 new cases, including an explicit `not.toContain('undefined')` regression pin for the bankruptcy-toast bug). Live browser click-through: fresh game boots at v3.1.8 with zero new console errors; state-injected a player onto a regulatory fee space and drove a real dice-based fee action to confirm the `dice_roll`/`card_draw` log entries still render with the exact right text post-refactor.

## [3.1.7] - 2026-07-17

### Refactor: domain-event stage 3 — collapse the 9 dual-channel log+toast duplicates
Third stage of [docs/design/domain-events.md](docs/design/domain-events.md)'s plan. Nine game moments used to fire from 2–3 separate hand-written `loggingService.info`/`notificationService.notify` call sites each (one for the permanent log, one for the toast, sometimes 3 near-duplicate notifies for the same moment). Each now fires **one** typed `GameEvent` emission, consumed by two new dedicated subscribers — `LogWriter` (`src/services/LogWriter.ts`) and `ToastWriter` (`src/services/ToastWriter.ts`) — so log and toast for the same moment can never again disagree or drift apart. Both are plain `GameEventBus` subscribers instantiated once in `ServiceProvider.tsx` (and `tests/ghost/bootstrapServices.ts` for headless test parity) — no new buffering machinery needed: `LoggingService`'s existing exploration-session mechanism already tags every `info()` call made during an active turn with the session id and discards it on Try Again unless explicitly committed, so `LogWriter` inherits Try Again correctness for free just by calling `loggingService.info()` like every migrated call site did.

Six new `GameEvent` variants added (`turn_committed`, `turn_discarded`, `dice_rolled`, `routed_back_to_review`, `manual_action_completed`, `recurring_card_effect_applied`); three moments (`FundingReceived`, `LifeEventTriggered`, `MovementCompleted`) reuse existing stage-2 event types, only losing their now-redundant standalone `notify()`/log calls. The trickiest migration — TurnDiscarded/TryAgainUsed — keeps the single emission at the exact point the old log call sat (**before** the turn-transaction discard), so `LogWriter`'s forced `isCommitted:true` override still survives the discard exactly as the old code's explicit override did; verified via `tests/services/TurnService-tryAgainOnSpace.test.ts`'s existing "discards BOTH the state TEMP and the log session together" test before and after.

**Two corrections to the design doc's own assumptions, caught during implementation, not before:**
- **Movement's "never logged" premise was only half true.** The design doc (and this stage's own plan) assumed a completed move never reaches the permanent log. In fact `MovementService.movePlayer()` — called by every success branch in `MovementExecutor` — already writes its own "Moved from X to Y" entry directly. Only the **failure** branches (no destination found) had zero log coverage. `LogWriter`'s new `movement` case therefore only logs `success:false` — logging `success:true` too would have silently duplicated every single move in the Chronicle. Caught by a live browser check of `globalActionLog`, not by a test (no test asserted the entry count).
- **`RoutedBackToReview`'s three branches don't all use the same phrasing.** The `chosen_destination` branch (player picked at a multi-destination choice) originally said "chose", not "is being directed to" like the other two branches — a distinction the first `ToastWriter` implementation missed and collapsed into one string. Caught by the new `ToastWriter.test.ts` (written by copying the three original strings verbatim before deleting their call sites), not by manual review.

Also discovered mid-migration: **hand-rolled real-service test bootstraps that construct `StateService`+`LoggingService` directly (not via `ServiceProvider` or the shared `bootstrapHeadlessServices`) need `LogWriter`/`ToastWriter` wired in too**, or a migrated call site's event fires into the void in that test's environment. Found via two real full-suite failures (`TurnService-tryAgainOnSpace.test.ts`'s Try-Again log-survival assertion, `E2E-03_ComplexSpace.test.ts`'s try-again log lookup) before proactively wiring the remaining dozen E2E/regression test files that construct real services the same way, rather than discovering the rest one 10-minute suite run at a time.

Verified: typecheck clean, production build clean, full suite **2405/2406 passed (1 pre-existing skip, zero failures)**, all 5 ghost gates green including both 50-game strict runs. New `LogWriter.test.ts`/`ToastWriter.test.ts` (26 tests) assert every event type's exact derived string, copied from each original call site before it was deleted. Live browser click-through: a full turn (manual action → dice-conditional card draw → scope lock → movement → turn end) produced exactly the right `globalActionLog` entries in the right committed/uncommitted state at each point, with the toast banners appearing correctly and no new console errors.

## [3.1.6] - 2026-07-17

### Refactor: domain-event stage 2 — typed `GameEvent` bus replaces flat `AutoActionEvent` (behavior-identical)
Second stage of [docs/design/domain-events.md](docs/design/domain-events.md)'s plan. The prototype event system — `AutoActionEvent`, one flat interface with ~15 optional fields shared across all 8 event types, dispatched via `StateService.emitAutoAction`/`subscribeToAutoActions` — is now a discriminated union (`src/types/GameEvents.ts`: `DiceConditionalCardEvent | LifeEventEvent | SeedMoneyEvent | AutomaticFundingEvent | MovementEvent | RoutingExplanationEvent | AutoDiceRollEvent | ApprovalRevokedEvent`) dispatched through a dedicated `GameEventBus` class (`src/services/GameEventBus.ts`) that `StateService` owns and delegates to via renamed `emitGameEvent`/`subscribeToGameEvents` methods. Each variant now only exposes the fields that event type actually carries — the old flat shape let `GameLayout`'s router read fields (e.g. `cardId` on a `movement` event) that a `movement` payload never sets, with no compiler check either way.

Migrated all 15 emitter call sites (CardEffectHandler ×2, CardService ×2, FinancialEffectHandler ×2, ManualActionProcessor, EffectEngineService, MovementService, TurnService, MovementExecutor ×4, SpaceArrivalProcessor) and both consumers (`GameLayout.tsx`'s modal router, `TVDisplay.tsx`'s dramatic-overlay router) in the same commit — per the design doc's do-not rule, no parallel old+new path (the `subscribeWithSelector` lesson: an optional migration never gets adopted). `LifeEventEffectSummary` moved alongside the new union but is re-exported from `StateService.ts` so its 5 existing consumers (`LifeEventModal.tsx`, `lifeEventReceipts.ts`, etc.) needed no changes. One quirk preserved deliberately, not "fixed": `FinancialEffectHandler` emits `life_event` for GAME OVER / BANKRUPTCY narrative (no `cardId`) alongside its real card-draw use (with `cardId`) — `GameLayout`'s modal gate already only acted on the latter, and `TVDisplay`'s overlay shows both; stage 2 is behavior-identical by design, so this dual-purpose typing carries over as-is (a candidate for stage 4's ownership move, not this stage).

Verified: typecheck clean, production build clean, 310 targeted tests (StateService/TurnService/MovementExecutor/MovementService/SpaceArrivalProcessor/FinancialEffectHandler/CardEffectHandler/CardService/EffectEngineService) green, full suite 2379/2380 passed (1 pre-existing skip, **zero failures** — cleaner than v3.1.5's baseline, all 5 ghost gates green including both 50-game strict runs). Live browser click-through: game start, player add, and two manual actions that route through the new `emitGameEvent` call sites (Hire Expeditors, Get Work Packages) completed with no console errors. (Their result modals didn't dismiss via click/Escape in this session's automation harness — the pre-existing "dice-result modal won't dismiss" tooling gap already tracked in TODO.md, not a regression from this change.)
...
The maintainer's long-term goal is handing the engine to a different community (e.g. a fantasy/D&D reskin) who edits ONLY CSV data, zero code. The 2026-07-12 audit found 5 places where real-world NYC strings were baked into program logic — rename a space or reword flavor text in the CSV and the game would silently break. All 5 are now data-driven, with the current NYC game verified behavior-identical (full suite 2370 passed; the only 2 full-suite failures were environmental — the known Windows resolved-dir EPERM under parallel load and one suite-load timeout — both pass clean in isolation):

1. **`approval_role` column (Spaces.csv → GAME_CONFIG.csv)** — which space acts as the DOB plan exam, FDNY plan exam, DOB audit, and DOB final review is now declared per-space in data (`dob_exam` / `fdny_exam` / `dob_audit` / `dob_final_review`). `ApprovalService` resolves roles via `DataService.getSpaceByApprovalRole()` / `getApprovalRole()` instead of comparing against hardcoded `REG-DOB-*` name constants; `DiceRollProcessor` and `MovementService` route through the same lookups (including one comparison a past Workstream 6 cleanup missed). The old exported constants remain as deprecated fallbacks so nothing external breaks.
2. **`npc_speaker` column (Spaces.csv → GAME_CONFIG.csv)** — which character narrates each space (feeding portraits, badges, and TTS voice across 8 files) is now assignable in data, with `pm` marking the 5 PM-voiced spaces. `characters.ts`'s `CHARACTER_MAP`/`PM_VOICED_SPACES` prefix-guessing remains only as a fallback for rows with no explicit speaker.
3. **New `CARD_TYPES.csv` (CLEAN_FILES)** — card-type display names ("Work Package", "Bank Loan", "Investor Funding", "Life Event", "Expeditor") now load from data via `DataService.getCardTypeLabel()`; `theme.ts` / `cardTypeNames.ts` / `costPreview.ts` consume it, keeping their old literals only as load-failure fallbacks.
4. **Work-scope flags (CARDS_EXPANDED.csv: `is_groundwork` / `is_utility` / `is_high_profile`)** — the 3 conditional Life Event cards (L012 soil contamination, L029 utility delay, L041/L044 high-profile) now gate on per-card Yes/No flags instead of `CardService`'s exact real trade-name string sets (`GROUNDWORK_WORK_TYPES` etc.). 56 W-cards were auto-flagged by applying the old sets one final time at CSV-generation, so current-game behavior is provably unchanged; a reskin just sets the flags on its own cards.
5. **`LOAN_TIERED` effect type (SPACE_EFFECTS.csv)** — the bank's tiered loan-fee logic (1% under $1.4M / 2% to $2.75M / 3% above) was triggered by `FinancialEffectHandler` string-matching the fee description for "1.4m"/"2.75m"; it's now a first-class effect type declared in data, so rewording the description can no longer silently break fee math. Also removed `CardService`'s dead investor-payout fallback that guessed amounts from card names ("angel investor" → $50K etc.) — every I-card has a real `investment_amount`, and a data-driven engine must not invent values from flavor text.

Pipeline + integration: `processGameData.js` carries all new columns/rows through CSV regeneration; verified the Data Editor's save round-trip (`_extraColumns` capture), the board-layout drag-save, and the classroom instance bake all preserve the new columns end-to-end. Typecheck, production build, and the ghost gates are green.
...
The maintainer sent a real photo of their TV hung indefinitely on the plain "Loading Unravel Codes..." screen — some TV browsers apparently hang trying to run the app rather than throwing a script error the existing `index.html` error-fallback handler could catch, and a TV has no dev console to see why. The 10-second "did React load?" check already existed in `index.html` but only did a silent `console.warn` — invisible to anyone without a keyboard and dev tools, i.e. every TV user. Extended it to reveal a visible on-screen tip after 10 seconds if the loading screen is still showing, suggesting the visitor try a different TV browser — naming **TV Bro**, which the maintainer confirmed works on their own TV where the default one hangs. Verified in an isolated sandbox copy (script deliberately removed so the app never mounts) that the tip stays hidden before 10s and becomes visible with the correct text at 10s, via direct DOM/computed-style inspection (screenshot tooling in this environment is unreliable, a known limitation).

### Docs: corrected an ISP misattribution from the v3.0.139/v3.0.140 write-ups
The v3.0.139 IPv6 home-alert fix and its code comments guessed the maintainer's reported IPv6 address was Comcast/Xfinity-range from general knowledge, without checking — the maintainer corrected this (they're on Verizon). Looked it up properly this time via an ARIN RDAP query on the actual address, which confirms the block is registered to Verizon. Corrected the CHANGELOG v3.0.139 entry and the comment in `server/homeIP.js`; no behavior changed, the fix itself didn't depend on which ISP it was.
...
Real-hardware follow-up on the v3.0.135 TV zoom fix (fb:feedback-1783998607682-3f9f2831 / fb:feedback-1783997840419-e121c34e), reported live by the maintainer on a Hisense 75" (4-years-old, VIDAA-class smart-TV browser): after last session's "make it bigger to read from the couch" fix shipped, the setup screen now showed *less* — the maintainer could no longer see the player tiles at all. Investigation (via the dashboard's own stored screenshots for the two original reports, not a simulated TV — no real TV hardware exists in this environment) found the original diagnosis backwards: the complaint was never about text being too small to read, it was that the TV's own usable screen was already tighter than a normal monitor's, and the v3.0.135 zoom made that worse, not better.

Two real, compounding bugs found and fixed in `PlayerSetup.tsx` / `PlayerList.tsx`:

1. **The zoom-vs-`vh` mismatch.** TV mode's "10-foot UI" zoom (`TV_MODE_ZOOM = 1.3`) works by pre-shrinking the container's own declared width/height (to `100/1.3 vw`/`vh`) so the zoom transform lands back at exactly 100vw/100vh. But `vh`/`vw` units always resolve against the *true* device viewport, not the shrunk local box they're nested in — so every `clamp(rem, vh, rem)`-based chrome element above the player list (header, hero logo/title, section labels) kept its same absolute size while the box containing it got smaller, eating a disproportionately bigger share of the available screen on TV than on PC. Fixed by giving the header, logo, title, and "Players" section fixed (non-`vh`) sizing specifically in TV mode, and merging the "👥 Players" heading with the player-count line into one compact row. Kept `TV_MODE_ZOOM` itself — the maintainer noted a user-adjustable zoom could be useful on other screens later, so the mechanism stays, just no longer starved of room by its own math on this screen.
2. **The bigger one — a responsive-grid packing bug in `PlayerList.tsx`.** The player-tile grid (`gridTemplateColumns: repeat(auto-fit, minmax(min(100%, 360px), 1fr))`) let auto-fit pack as many 360px+ columns as would fit. But a tile's own two halves (name/avatar/color-picker block + QR code block) need roughly 500px to sit side by side — so at some screen widths (confirmed at a real 1920px TV width) auto-fit packed 3 columns of ~447px each, just under that true minimum: each tile's QR silently wrapped below its name/avatar *inside the tile*, nearly doubling every tile's height and pushing tiles off the bottom of the screen with the forced-visible scrollbar as the only clue. Raised the column floor to 520px (380px for TV's now-more-compact tiles) so tiles can no longer self-wrap this way, and gave TV-mode tiles a smaller footprint (76px QR instead of 100px, smaller avatar/swatches/padding via a new `compact` prop) so more fits per screen without shrinking the QR below a comfortably phone-scannable size.

Also dropped the "Every player joins from their phone…" instructional line from TV mode — each tile's own "⚠ Required: scan to join" label already says the same thing per-player, and the line cost a full row of scarce vertical space for redundant information.

Verified via typecheck, a clean production build, and live DOM measurement (comparing actual content height against visible area with exactly 4 players added) at two TV-plausible logical viewports: 1920×1080 (fits with zero scrolling, exact match) and 1280×720 — the more conservative case, chosen because the maintainer's real TV is an older Hisense running VIDAA-class software, known for reporting a lower logical resolution than its physical 4K panel (also fits with zero scrolling after this round). PC-mode setup at a normal desktop width (1440px) confirmed unaffected — same card size/layout as before. No dedicated test suite covers this screen (confirmed via search — this is a pre-existing gap, not introduced here), consistent with this file's prior sessions.
...
Maintainer request, after playtesting the fb:f453b1f3 A/B experiment: "we can now remove the classic player pannel and make the light version act like black" — i.e., retire the classic panel entirely, and since the dark-mode press-and-hold `TurnCommitControl` won the A/B test, stop gating it to dark mode only.

**Classic panel removal.** Deleted `ActionCenterPanel.tsx` and everything exclusively used by it: 8 section components (`FinancesSection`, `ProjectScopeSection`, `EventsSection`, `StoryAccordion`, `TimeSection`, `PlayerLogSection`, `ProjectLedger`, `ApprovalBadges`) plus their CSS and tests, `BeforeAfterBlock.tsx` (DiceResultModal's classic-only outcome block — the new-view `OutcomeChangesV2` is now unconditional), and the classic/new version toggle itself (`PanelVersion`/`usePanelVersion`/`getStoredPanelVersion` in `panelTheme.ts`, plus the "Try new design / Back to classic" button in `PlayerPanelWrapper.tsx`). `PlayerPanelV2` is now the only panel. Also swept two already-fully-dead leftovers found during the audit (`CurrentCardSection`, `StorySection` — zero real importers anywhere, not even from classic). The shared types classic used to export (`ReferenceTab`, `TabRequest`, `ActionCenterPanelProps` → renamed `PlayerPanelProps`) moved to a new `panelTypes.ts` so `PlayerPanelV2`/`PlayerPanelWrapper`/`GameLayout`/`relatedTab.ts` keep working without importing a component that no longer exists. `ActionCenterPanel.css` survives — the Space Data Editor's live preview (`PlayerPreviewPanel.tsx`) genuinely depends on its `action-center__*` classes — renamed to `PlayerPreviewPanel.css` and moved to sit next to its one remaining consumer. Main JS bundle shrank 465KB → 397KB (gzipped ~117KB → ~103KB).

**TurnCommitControl unification.** `PlayerPanelV2`'s footer no longer gates the merged control on `mode === 'dark'` — it renders identically in both themes now (styled via its own `mode` prop, which already supported light). The retired separate-buttons "Option A" design (`TurnCostToggle.tsx`) is deleted. When a space has no negotiate option at all, the fallback is a single plain commit button (unchanged from before, just no longer wrapped in a now-pointless flex row).

**Test fallout, fixed properly, not papered over:**
- `DiceResultModal.test.tsx`'s classic-path suppression test (forced `localStorage.ucPanelVersion = 'classic'` to exercise the now-deleted `BeforeAfterBlock`) rewritten to assert the same real regression guard — no duplicate-rendered delta between the Effects row and the snapshot table — against `OutcomeChangesV2`'s actual compact-money format ("+$50K") and unicode-minus convention.
- `PlayerPanelV2.test.tsx` updated: the old `TurnCostToggle` empty-state text ("Nothing to report") no longer exists once `TurnCommitControl` always shows all 5 categories with placeholders (v3.0.131) — updated to check for the placeholder instead.
- `E2E-01_HappyPath.test.tsx`: a real regression, caught by the full suite, not by inspection. The test found "End Turn"/"Take the check" via `getByRole('button', ...)` and clicked once — but a negotiable space's commit control is now `role="tab"` requiring a ~650ms press-and-hold, not a click. Added a `pressAndHoldToCommit()` helper (real `pointerDown` → 700ms real wait → `pointerUp`, this file doesn't use fake timers) and switched the queries to `getByRole('tab', ...)`. Fixing this properly then exposed a SECOND, pre-existing latent issue: the test's final assertion expected `globalTurnCount` to stay at 1 after two separate completed End Turn actions, which contradicts `StateService.advanceTurn`'s documented one-increment-per-completed-turn semantics (confirmed via a temporary debug trace: the count was already 1 immediately after the FIRRST action alone) — corrected to `toBe(2)`, the actually-correct value; the original assertion likely never really held and was passing via a timing artifact of the old synchronous click, not because 1 was ever the right number.

Verified via typecheck, build, and three full-suite runs across the three stages of this change (classic removal alone: 2330/2333, only pre-existing flaky failures; +unification: 1 new failure isolated and fixed; final combined: 2331/2333, the one remaining failure a different random `E2E-AllPaths.test.ts` sub-test timeout — the same already-documented non-deterministic resource-contention flakiness from full-suite load, not a regression). Live browser session confirmed both light and dark mode now render the identical merged control, the classic/new toggle button is gone, and neither mode shows the error-fallback screen.
...
Second same-day playtest follow-up on `TurnCommitControl` (fb:f453b1f3 thread): (1) "the negotiate and end turn buttons seem to jump" — traced to `endCostLine`, an optional one-line cost summary that only rendered on the End side, making the preview box one line taller there than on Try Again and visibly shifting the buttons below it. Now always reserved as a fixed-height (`minHeight`) line regardless of side — real text on End, an invisible placeholder (`visibility: hidden`, non-breaking space) on Try Again — so the box is identically sized both ways. (2) "some sort of indicator showing that the data is coming from the button that was tapped" — the preview is now a comic-bubble OVERLAY (`position: absolute`, doesn't push the buttons — which also fully eliminates the jump at the root, independent of fix #1) with a small triangular tail that slides between sitting under the Try Again tab (25%) and the End tab (75%) as the selection changes, both a visual anchor ("this data came from that button") and a swipe-like motion cue. (3) "maybe the bubble should not always be there... only appear when tapped and show up for 3 seconds then disappear" — the bubble is now hidden by default (`opacity: 0`, `pointer-events: none`, `aria-hidden`) and only appears for 3 seconds after any tap, hold-start, or keyboard compare (Space), fading back out on its own; a fresh interaction resets the countdown rather than letting an in-flight timer cut it off early. Verified via typecheck, build, an expanded test suite (14/14 in `TurnCommitControl.test.tsx`, including new coverage for hidden-by-default, the 3-second auto-hide, countdown-reset-on-fresh-tap, and the fixed-height cost line), the full player-component suite (711/711), and a live browser session confirming via DOM/attribute inspection: the bubble starts hidden, appears on tap, the tail slides 25%→75% between sides, the cost line's `visibility` toggles without changing the reserved space, and the bubble auto-hides after the full 3-second window (computed-style opacity reads were unreliable in this environment — a known, already-documented tooling limitation — so inline-style/attribute inspection was used instead, consistent with v3.0.131's verification).

## [3.0.131] - 2026-07-14

### Dark-mode commit control (fb:f453b1f3 A/B) — three tweaks after maintainer playtest picked it as the winner
Maintainer played both variants and picked the dark-mode press-and-hold `TurnCommitControl` (v3.0.128/129) as the clear winner, with three follow-up complaints: (1) the cost preview sat below the buttons, so the player's own thumb — pressing the button to hold-commit — covered the exact info that was supposed to be changing; (2) rows appeared/disappeared depending on which categories a space touched, so the layout jumped around rather than staying legible; (3) the growing hold-progress line sat at the very bottom of the button, also directly under the thumb. Fixed all three: (1) reordered the preview panel to render ABOVE the tablist, so it's never under the pressing thumb; (2) added `toFullRowSet()` to `costPreview.ts` — expands a populated-only row list into all 5 categories (Labor/Work/Expediting/Money/Time) always, filling any empty one with a "—" placeholder, so only the value column moves between spaces (kept as a separate helper, not a change to the two exported preview functions, so the light-mode `TurnCostToggle` keeps its original populated-only + "Nothing to report" behavior); tightened row padding/font (10.5px→10px, 1.5px→1px vertical padding) to compensate for the now-fixed 5-row height; (3) replaced the bottom progress bar with an SVG outline that sweeps around the button's full perimeter (`stroke-dasharray`/`stroke-dashoffset` with `pathLength=100` for simple percentage math, `vector-effect="non-scaling-stroke"` to keep the line width visually constant despite the non-uniform `viewBox` stretch) — parts of the outline stay visible past the fingertip regardless of exactly where the player is pressing. Verified via typecheck, build, updated/expanded test suite (10/10 in `TurnCommitControl.test.tsx` + 4 new in `costPreview.toFullRowSet.test.ts`), the full player-component suite (707/707), and a live browser session: confirmed the preview/hint/buttons DOM order via page-text extraction, confirmed all 5 rows render with correct values/placeholders for a real space, and confirmed the outline's `stroke-dashoffset`/`opacity` correctly flip on press (with the 650ms linear transition) and reset on early release (which also correctly did NOT fire a commit) via direct DOM/style diffing (screenshot tooling in this environment is currently unreliable — a known, already-tracked limitation — accessibility-tree/computed-style inspection substituted).
