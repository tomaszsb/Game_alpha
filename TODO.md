...
- [ ] **Extend dark/light mode beyond `PlayerPanelV2` to the rest of the board.** Today only `PlayerPanelV2` (and its footer's `TurnCommitControl`, unified v3.0.13x) actually respects the light/dark toggle. The board itself (ReactFlow spaces/edges/labels), `TVDisplay`, and at least two modals (`ChoiceModal`/`CardModal` — confirmed dark-unaware in the v3.0.127 CHANGELOG entry, only `DiceResultModal` got the fix) still render light-only regardless of the toggle. Needs a scoped audit: which surfaces read `panelPalettes` vs. hardcoded colors, and in what order to tackle them (board first vs. remaining modals first).
...
- [x] **Lift real-world hardcoding blocking a content-only reskin** — **SHIPPED v3.1.0 (2026-07-16).** All 5 blockers below are now CSV-driven (`approval_role` + `npc_speaker` columns, `CARD_TYPES.csv`, `is_groundwork`/`is_utility`/`is_high_profile` card flags, `LOAN_TIERED` effect type); current NYC game verified behavior-identical. Details: CHANGELOG 3.1.0. Original scope kept below for reference. Maintainer's long-term goal: hand the engine to a different community (e.g. a fantasy/D&D reskin) who edits ONLY CSV data, zero code. Audited 2026-07-12 — not ready yet; confirmed blockers: (1) `ApprovalService.ts:37-74` hardcodes DOB/FDNY space-name strings for approval-gate logic. (2) **Bigger, newly found:** `src/constants/characters.ts`'s `CHARACTER_MAP`/`PM_VOICED_SPACES` hardcode which NPC speaks at which space by name/prefix, feeding 8 files (portraits, badges, TTS voice) — renaming spaces in CSV would blank/mismatch every NPC board-wide. (3) `theme.ts` hardcodes card-type display labels ("Work Package" etc.) outside CSV. (4) `CardService.ts`'s `GROUNDWORK_WORK_TYPES`/`UTILITY_WORK_TYPES`/`HIGH_PROFILE_WORK_TYPES` gate 4 Life Event cards (L012/L029/L041/L044) on exact real trade-name strings. (5) Two spots string-match CSV flavor text directly (`FinancialEffectHandler.ts:452` checks for "1.4m"/"2.75m"; `CardService.ts:1952-1957` matches "angel investor"/"venture capital"/"crowdfunding") — these break on any flavor-text edit, not just a reskin. Most space-based movement logic is ALREADY CSV-driven (a past Workstream 6 cleanup fixed that half) — this is the remaining half. **Not** the REAL/TEMP-vs-ledger question below — audited as unrelated to this goal.
...
- [ ] **`TurnService.ts` is an overloaded orchestrator** (2191 lines, 16 methods spanning dice rolling, effect processing, manual effects, negotiation/Try Again, rerolling, and automatic funding — confirmed 2026-07-14, not just changelog-inferred). External review's proposed shape: a staged pipeline (Movement → Arrival → Automatic Effects → Player Decisions → Commit → Cleanup → Transition) instead of one class doing everything. Leaning toward this specific shape as the leading candidate (not locked) — it's a well-established pattern for turn-based engines, and critically, a generic staged pipeline is inherently more reusable for the maintainer's own long-term D&D-reskin goal than the current NYC-permitting-specific orchestrator (dice/negotiation/funding methods named and shaped around this game's rules). **Do NOT do casually** — same blast-radius caution as the TEMP/REAL unification item above; touches the most call-heavy service in the codebase.
