...
### Newly arrived (2026-05-23)
- [ ] **ResizeObserver loop warning surfaced as app error** — Browser fired "ResizeObserver loop completed with undelivered notifications" (benign Chromium warning) but the app's global error handler caught it and showed an error UI. Likely from React Flow's resize observer on BoardCanvas. Fix: filter this specific message in the window.onerror / ErrorBoundary so it doesn't trip the user-facing error path. Reported under v3.0.0 on G234. <!-- fb:feedback-1779511071011-241911b6 -->
