...
- [ ] **G160: per-edge waypoint redirect** — approved by the maintainer 2026-08-02, scoped 2026-08-02 (ready to build, no more design discussion needed). Original report (fb:feedback-1778327469678-d27a73d0): "I would want more control of which lines I want shown **and how they run**" — show/hide shipped v3.1.49; this is the "how they run" half, for an edge that must stay visible but whose auto-routed A* path (`SmartBezierEdgeTuned`, [BoardCanvas.tsx:421](../../src/components/board/BoardCanvas.tsx#L421)) detours somewhere confusing.

  **Data model:** new localStorage key `unravel:boardEdgeWaypoints`, `Record<edgeId, {x:number;y:number}>` — one waypoint per edge (covers "the detour looks wrong"; not building multi-point spline editing for a v1). Same tier/pattern as the existing `unravel:boardHiddenEdges` ([GameLayout.tsx:138-160](../../src/components/layout/GameLayout.tsx#L138)) — per-device, admin-only, not CSV/server-persisted. New `BoardCanvasProps`: `edgeWaypoints?`, `onSetEdgeWaypoint?`, `onClearEdgeWaypoint?`, wired in GameLayout next to the hidden-edges state.

  **Rendering:** new edge type `'waypoint'` registered in `edgeTypes` ([BoardCanvas.tsx:426](../../src/components/board/BoardCanvas.tsx#L426)) alongside `'smart'`. A `WaypointEdge` component draws two segments (source→point, point→target) via `getBezierPath` — bypasses A* entirely for that edge, so the admin's placement is authoritative, not a routing hint. In the edge-builder / `visibleEdges` memo, an edge with a stored waypoint gets `type:'waypoint'` + `data.waypoint`; everything else keeps `type:'smart'` unchanged.

  **Admin interaction:** the existing `onEdgeClick` ([BoardCanvas.tsx:757](../../src/components/board/BoardCanvas.tsx#L757)) hides on single-click. Redirect needs to coexist with that on the same edges — switch to `onEdgeMouseDown` + window `mousemove`/`mouseup`, with a ~5px movement threshold distinguishing click (hide, existing) from drag (place waypoint, new). Drop point converts via `useReactFlow().screenToFlowPosition` (already imported). Live-preview the 2-segment path during drag by updating local edge state on each mousemove, same pattern BoardCanvas already uses for hover-injected node data.

  **UI:** extend [BoardToggle.tsx](../../src/components/board/BoardToggle.tsx)'s existing "🚫 N hidden · restore" pill — lean toward ONE combined "reset" clearing both hidden + redirected sets rather than two separate pills, to avoid cluttering an admin-only control; confirm during build.

  **Explicitly out of scope for v1:** multiple waypoints/spline editing (one point covers the reported problem); server/CSV persistence (stays localStorage, matching the hide/show precedent — if that precedent changes later, e.g. to go classroom-wide, redirect should follow it, not lead it).

  **Estimate:** ~2-3h — new edge component, click-vs-drag discrimination, GameLayout/BoardToggle wiring, tests for the drag threshold + waypoint path math. <!-- fb:feedback-1778327469678-d27a73d0 -->
