...
- [ ] **🚨 TV setup: 4 players added, lower 2 cards clipped off-screen with no scroll** — v3.0.16 regression from the setup unification. Screenshot shows `4/4 players added · Maximum reached` with only Player 1 + Player 2 visible; their QR codes are clipped mid-screen. The PlayerList grid `repeat(auto-fit, minmax(min(100%, 360px), 1fr))` wraps cards to additional rows when needed, but the parent `playerListWrapper` doesn't have an `overflow-y: auto` scroll container at TV widths — content overflows the panel. Need to add a max-height + scroll on `.playerListWrapper` so the cards live inside a scrollable region. Roughly 5 min fix. <!-- fb:feedback-1779793772601-ffdddd29 -->
...
- [ ] **LOGIC_QUESTION asks for info the engine already has (approval state)** — Same playtest, Question 1 of 5 at REG-FDNY-FEE-REVIEW reads "Did you pass FDNY approval before?" — but the engine already tracks DOB/FDNY approval state via `ApprovalService` (W7 Phase 7.4). The chain should consult that state and auto-answer where possible, then either skip the question or proceed straight to the resolved branch. Two paths: (a) in MovementService.walkLogicChain, before calling `choiceService.createChoice`, check if the question matches a known approval key and auto-resolve; (b) author a `auto_answer_from` column in LOGIC_QUESTIONS.csv pointing at the approval flag, engine reads it. Path (b) is cleaner long-term. <!-- fb:feedback-1779810497226-58a2112b -->

- [ ] **L023 "Soil Contamination" applies +4 days regardless of groundwork** — Card text says *"If the current project involves groundwork increase the filing time by 4 days."* — but the engine has no notion of "project involves groundwork" and just applies the +4 unconditionally when the card fires. Either: (a) add a groundwork flag to the player state + condition the tick_modifier on it; (b) re-author the card to remove the condition; or (c) lift "involves groundwork" into the dice-conditional column system that already exists. Probably need a discussion before fixing — what's "groundwork" mean game-mechanically? <!-- fb:feedback-1779810408636-776e3ba7 -->

- [ ] **Life Event modal: "(rolled 1 at SPACE-NAME)" parenthetical exposes game machinery** — Voice rule violation. The modal's sub-banner currently reads "A major disturbance just hit the project. (rolled 1 at REG-DOB-TYPE-SELECT)" — both "rolled" and the raw space-id leak meta-language at the player. Should read in-character (e.g., narrator commentary on the situation) or just drop the parenthetical. Localized voice fix, no engine work. <!-- fb:feedback-1779809951376-1aad6035 -->

- [ ] **TV layout: hoist player indicator into blue header, shrink bottom subtitle** — Polish ask: the player strip (orange chip "Player 4 - $6160K · 6 cards" pulsing on the current player) currently sits below the blue header; user wants it MOVED INTO the blue header to reclaim ~50px of vertical space for the board. Also the bottom "The owner opens the books" subtitle is currently large — should be smaller. TVDisplay header + footer styling adjustment. <!-- fb:feedback-1779794652976-608bb670 -->

- [ ] **No way to add a phone mid-game on the TV (+ "cards" wording in player strip)** — Two issues from same report. (a) User started a TV game and forgot to log in via phone; once the game is past SETUP there's no affordance to summon the QR code and join a phone. The QR sidebar disappears at PLAY phase (v3.0.15 intentional). Need a "Connect phone" button somewhere — maybe in the player strip avatar context-menu, or a small QR-toggle in the header. (b) Player strip says "6 cards" — voice rule says no "card(s)" / "game" language. Should be role-specific count ("6 Expeditors" / "6 Permits" etc.) or drop the count entirely. <!-- fb:feedback-1779794024454-5dc01203 -->
