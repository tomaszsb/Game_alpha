...
- **Engine-data separation (Workstream 6 in BETA_PLAN_V3)** — Audit completed 2026-04-26: ~25 hardcoded space-ID references across 9 source files + 2 type-level hardcodes block the educator-editing vision. Full plan with 8 scenarios, per-scenario CSV columns, file-level refactor targets, test strategy, and risk analysis lives in [`docs/core/BETA_PLAN_V3.md`](docs/core/BETA_PLAN_V3.md) → **Workstream 6**. Total effort ~30-45 hours across 3 phases. Suggested first scenario: #8 (regulatory-phase override, XS effort, no new columns) to exercise the refactor pattern at minimum risk. Starts after voice rewrite ships v2.51.0.
