...
- [ ] **Bucket D — service-surface API narrowing (~10 sites).** `NegotiationService.initiateNegotiation(context: any)` / `completeNegotiation(agreement: any)` / `playerHasCard/removeCardsFromPlayer/addCardsToPlayer(player: any)` → `Player`; `StateService.updateNegotiationState(negotiationState: any)` → `NegotiationState` + 3× `(result as any).committedState/updatedState`; `DiceRollProcessor.gameState: any` → `GameState`; `GameLayout` `useState<any>(null)` → `DiceResultEffect | null`; `GameRulesService.extractValidDestinations(movement: any)` → `Movement`. Needs careful review around transaction-result types.
