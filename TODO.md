...
- [x] ~~Delete orphaned dead-code cluster~~ — DONE 2026-07-18 (v3.1.9, see CHANGELOG). Deleted the audited five plus the rest of the boundary the deletion audit surfaced: `CardContent.tsx` (only CardModal imported it), the whole `PlayerActionService` class (playCard was its last method), `dismissModal`, the `ActiveModal` type + `GameState.activeModal` field, and `shouldShake`'s card-type branch. The unreachable "Permanent-duration card throws mid-discard" bug and v3.1.8's flagged possible double-activate in `PlayerActionService.playCard()` both died with the code. One NEW latent gap found during the test migration — manual `cardService.playCard` of a Global-scope no-duration card double-fanned effects (N×N; Kid E's guard only covered the auto path) — was FIXED same-day in v3.1.10 (guard extended to all paths + red-green-verified regression test in E2E-05).
