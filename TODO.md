...
- [x] **Domain event architecture — all 4 planned stages SHIPPED.** Full design at [docs/design/domain-events.md](docs/design/domain-events.md). Stage 1 (delete dead `TurnService.takeTurn()`) SHIPPED v3.1.5. Stage 2 (typed `GameEvent` discriminated union + `GameEventBus`) SHIPPED v3.1.6. Stage 3 (collapsed the 9 dual-channel log+toast duplicates into `LogWriter`/`ToastWriter`) SHIPPED v3.1.7. Stage 4 (moved emitter ownership into ApprovalService/FinancialEffectHandler/CardService; fixed a real bankruptcy/design-fee-cap toast bug — the old `life_event` repurposing rendered "Received: undefined"; gave the WIN ending its first-ever announcement; promoted CardService's 12 card-lifecycle log calls to typed events; merged CardPlayed+CardActivated) SHIPPED v3.1.8. Stage 5 (optional UI-subscription migration to specific events instead of blanket `stateService.subscribe()`) remains — explicitly gated on a measured render-cost problem, not scoped speculatively; may never happen. The notification-bus and EffectExecutor items above can now be re-scoped independently.
- [ ] **`PlayerActionService.playCard()` possible double-activate bug** (found during domain-event stage 4, 2026-07-17, NOT fixed): this is the classic-panel's independent card-play path (`CardActions.tsx`) — it never calls `CardService.playCard()`, instead building effects via `EffectFactory.createEffectsFromCard` (which emits its own `CARD_ACTIVATION` effect when `card.duration_count > 0`) AND separately triggering `cardService.finalizePlayedCard()` via a `PLAY_CARD` effect. Static trace suggests a duration card played through this path could call `activateCard()` twice — once from the explicit `CARD_ACTIVATION` effect, once from `finalizePlayedCard`'s own duration re-check — producing a duplicate `card_activated` log entry. Not confirmed by a live run; needs a targeted repro (play a duration card, e.g. an L-card with `duration_count>0`, via the classic panel specifically) before deciding whether it's real.
