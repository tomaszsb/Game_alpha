...
- [ ] **Delete orphaned dead-code cluster: `CardModal.tsx`, `CardActions.tsx`, `CardsSection.tsx`+`.css`, `PlayerActionService.playCard()`, `StateService.showCardModal()`** (audited 2026-07-17: `showCardModal` — the only thing that ever opens `CardModal` — has zero callers anywhere in `src/`; leftover from the v3.0.128-137 classic-panel removal that deleted the panel but not this orphaned modal/section pair). Contains a real, confirmed, but unreachable bug (a "Permanent"-duration card, e.g. any Bank Loan/Investment/Work Package, throws `"Card not found in player hand"` mid-discard since `EffectFactory`'s `CARD_ACTIVATION` effect already moved it out of hand before `finalizePlayedCard` tries to discard it) — not worth fixing, worth deleting alongside the cluster.
