...
- [ ] **Domain event architecture — DESIGN PASS DONE 2026-07-17, ready for staged implementation.** Full design at [docs/design/domain-events.md](docs/design/domain-events.md) (event catalog grounded in a verified call-site audit, staged plan, open questions, do-not rules). Key finding: `AutoActionEvent`/`emitAutoAction` in StateService is already the prototype — the work is promoting it to a typed discriminated-union bus and collapsing the 9 moments currently announced twice (log + toast, hand-written separately). Stage 1 (delete dead `TurnService.takeTurn()`) SHIPPED v3.1.5. Remaining stages, each independently shippable: (2) typed `GameEvent` union + bus, migrate existing AutoActionEvent traffic; (3) collapse the 9 dual-channel duplicates via LogWriter/ToastWriter subscribers (needs turn-transaction buffering so Try Again still discards log entries); (4) move emitter ownership into ApprovalService/FinancialEffectHandler/CardService; (5) optional UI-subscription migration — only on measured render pain. Stages 2–4 ≈ one session each; model routing per 2026-07-17 discussion: Sonnet for 2/4/5, Opus optional for 3. The notification-bus and EffectExecutor items above sit on top of this — re-scope them after stage 4.
