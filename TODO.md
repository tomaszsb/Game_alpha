...
- [ ] **Domain event architecture — stages 1–3 SHIPPED, stage 4 next.** Full design at [docs/design/domain-events.md](docs/design/domain-events.md) (event catalog grounded in a verified call-site audit, staged plan, open questions, do-not rules). Stage 1 (delete dead `TurnService.takeTurn()`) SHIPPED v3.1.5. Stage 2 (typed `GameEvent` discriminated union + `GameEventBus`, migrating all 15 `AutoActionEvent` emitter call sites + GameLayout/TVDisplay consumers) SHIPPED v3.1.6. Stage 3 (collapsed the 9 dual-channel log+toast duplicates into `LogWriter`/`ToastWriter` bus subscribers; added genuinely-new log coverage for failed movement; found and fixed two places the design doc's own assumptions were wrong — movement-success was already logged elsewhere, and one RoutedBackToReview branch had distinct wording the first pass missed) SHIPPED v3.1.7. Remaining stages, each independently shippable: (4) move emitter ownership into ApprovalService/FinancialEffectHandler/CardService; (5) optional UI-subscription migration — only on measured render pain. Model routing per 2026-07-17 discussion: Sonnet for 4/5, Opus optional for 3 (3 is done). The notification-bus and EffectExecutor items above sit on top of this — re-scope them after stage 4.
