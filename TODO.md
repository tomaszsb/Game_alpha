...
- [ ] **Domain event architecture — stages 1–2 SHIPPED, stage 3 next.** Full design at [docs/design/domain-events.md](docs/design/domain-events.md) (event catalog grounded in a verified call-site audit, staged plan, open questions, do-not rules). Stage 1 (delete dead `TurnService.takeTurn()`) SHIPPED v3.1.5. Stage 2 (typed `GameEvent` discriminated union + `GameEventBus`, migrating all 15 `AutoActionEvent` emitter call sites + GameLayout/TVDisplay consumers, old flat type removed in the same commit) SHIPPED v3.1.6. Remaining stages, each independently shippable: (3) collapse the 9 dual-channel duplicates (log call + toast call for one moment) via LogWriter/ToastWriter subscribers — needs turn-transaction buffering so Try Again still discards log entries; (4) move emitter ownership into ApprovalService/FinancialEffectHandler/CardService; (5) optional UI-subscription migration — only on measured render pain. Stages 3–4 ≈ one session each; model routing per 2026-07-17 discussion: Sonnet for 4/5, Opus optional for 3. The notification-bus and EffectExecutor items above sit on top of this — re-scope them after stage 4.
