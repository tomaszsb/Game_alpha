...
- [ ] **TV setup screen doesn't fit / "resolution is crap" on a 4K TV.** <!-- fb:feedback-1783998607682-3f9f2831 --> <!-- fb:feedback-1783997840419-e121c34e --> Two reports filed ~13 min apart (v3.0.130), almost certainly the same tester: "Too low resolution on TV" (trying to see the player panel) and "Can't go back up — these have to fit on a screen, this TV is 4k but resolution is crap" (setting up). Reads like the setup screen's CSS px/rem sizing doesn't scale up for an actual large 4K display at normal viewing distance (looks tiny), plus a scroll/navigation snag in TV mode's forced-visible player-list scrollbar (`selectedMode === 'tv' ? 'scroll' : 'auto'` in `PlayerSetup.tsx`). Needs investigation on real TV hardware — not auto-built, can't reproduce 4K-at-a-distance scaling in this dev environment.
