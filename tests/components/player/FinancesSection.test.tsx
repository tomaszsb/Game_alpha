...
describe('FinancesSection', () => {
  ...
  const mockPlayer: Player = {
    ...
    costHistory: [
      { category: 'regulatory', amount: 500, description: 'Bank processing fee', cardId: 'BANK-FEE', timestamp: new Date().toISOString() },
      { category: 'architectural', amount: 15000, description: 'Architectural design', cardId: 'ARCH-001', timestamp: new Date().toISOString() },
      { category: 'architectural', amount: 5000, description: 'Additional design work', cardId: 'ARCH-002', timestamp: new Date().toISOString() },
      { category: 'regulatory', amount: 10000, description: 'DOB filing fee', cardId: 'REG-001', timestamp: new Date().toISOString() }
    ]
  };
  ...
  describe('Cost Tracking Display', () => {
    ...
    it('should display regulatory fees category when costs exist', () => {
      render(<FinancesSection {...defaultProps} />);
      ...
      expect(screen.getByText('Regulatory Fees')).toBeInTheDocument();
      // Use getByRole with name pattern to find the specific button (playerWithCosts has regulatory: 500)
      expect(screen.getByRole('button', { name: /Regulatory Fees.*500/i })).toBeInTheDocument();
    });
    ...
    it('should display architectural fees category', () => {
      ...
      // playerWithCosts has architectural: 15000
    ...
    it('should expand cost category to show transaction details when clicked', () => {
      ...
      // Find and click the Regulatory Fees category header (playerWithCosts has regulatory: 500)
      const regulatoryFeesHeader = screen.getByRole('button', { name: /Regulatory Fees.*500/i });
      fireEvent.click(regulatoryFeesHeader);

      // Should now show the transaction details from playerWithCosts.costHistory
    ...
    it('should handle backward compatibility (player without cost structures)', () => {
      const legacyPlayer = {
        ...mockPlayer,
        money: 1000,
        // Explicitly remove cost structures for legacy player
        costHistory: undefined,
        costs: undefined,
        expenditures: undefined,
        moneySources: undefined
      };
      ...
      // Should not show cost sections for legacy player
    ...
    it('should collapse cost category details when header clicked twice', () => {
      ...
      // Expand the FINANCES section first
      const header = screen.getByRole('button', { name: /FINANCES/i });
      fireEvent.click(header);
    ...
    it('should display all cost entries for a category when expanded', () => {
      const playerWithMultipleCosts: Player = {
        ...
        costs: {
          ...
          regulatory: (playerWithCosts.costs?.regulatory || 0) + 500
        }
      };
      ...
      const regulatoryFeesHeader = screen.getByRole('button', { name: /Regulatory Fees/i });
      fireEvent.click(regulatoryFeesHeader);

      // Should show both entries
      expect(screen.getByText('Regulatory fee 1')).toBeInTheDocument();
      expect(screen.getByText('Regulatory fee 2')).toBeInTheDocument();
